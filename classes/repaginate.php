...
/**
 * Defines the assignmentques repaginate class.
 *
 ...
 *
 * The assignmentques setting allows users to write assignmentqueszes with one question per page,
...
class repaginate {
    ...
    /** @var int the id of the assignmentques being manipulated. */
    private $assignmentquesid;
    /** @var array the assignmentques_slots for that assignmentques. */
    private $slots;

    /**
     * Constructor.
     * @param int $assignmentquesid the id of the assignmentques being manipulated.
     * @param stdClass[] $slots the assignmentques_slots for that assignmentques.
     */
    public function __construct($assignmentquesid = 0, $slots = null) {
        global $DB;
        $this->assignmentquesid = $assignmentquesid;
        if (!$this->assignmentquesid) {
            $this->slots = array();
        }
        if (!$slots) {
            $this->slots = $DB->get_records('assignmentques_slots', array('assignmentquesid' => $this->assignmentquesid), 'slot');
        } else {
            $this->slots = $slots;
        }
    }
    ...
    public function repaginate_slots($nextslotnumber, $type) {
        ...
        foreach ($this->slots as $slot) {
            if ($slot->slot < $nextslotnumber) {
                ...
                // Update DB.
                $DB->update_record('assignmentques_slots', $nextslot, true);
    ...
    /**
     * Repaginate the rest.
     * @param stdClass[] $assignmentquesslots
     * @param int $slotfrom
     * @param int $type
     * @param bool $dbupdate
     * @return stdClass[]
     */
    public function repaginate_the_rest($assignmentquesslots, $slotfrom, $type, $dbupdate = true) {
        global $DB;
        if (!$assignmentquesslots) {
            return null;
        }
        $newslots = array();
        foreach ($assignmentquesslots as $slot) {
            if ($type == self::LINK) {
