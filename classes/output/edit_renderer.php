...
/**
 * Renderer outputting the assignmentques editing UI.
 *
...
namespace mod_assignmentques\output;
defined('MOODLE_INTERNAL') || die();

use \mod_assignmentques\structure;
use \html_writer;
use renderable;
...
class edit_renderer extends \plugin_renderer_base {
     ...
     *
     * @param \assignmentques $assignmentquesobj object containing all the assignmentques settings information.
     * @param structure $structure object containing the structure of the assignmentques.
     * @param \question_edit_contexts $contexts the relevant question bank contexts.
     * @param \moodle_url $pageurl the canonical URL of this page.
     * @param array $pagevars the variables from {@link question_edit_setup()}.
     * @return string HTML to output.
     */
    public function edit_page(\assignmentques $assignmentquesobj, structure $structure,
            \question_edit_contexts $contexts, \moodle_url $pageurl, array $pagevars) {
        $output = '';

        // Page title.
        $output .= $this->heading_with_help(get_string('editingassignmentquesx', 'assignmentques',
                format_string($assignmentquesobj->get_assignmentques_name())), 'editingassignmentques', 'assignmentques', '',
                get_string('basicideasofassignmentques', 'assignmentques'), 2);

        // Information at the top.
        $output .= $this->assignmentques_state_warnings($structure);

        $output .= html_writer::start_div('mod_assignmentques-edit-top-controls');
        $output .= $this->assignmentques_information($structure);
        $output .= $this->maximum_grade_input($structure, $pageurl);

        $output .= html_writer::start_div('mod_assignmentques-edit-action-buttons btn-group edit-toolbar', ['role' => 'group']);
        $output .= $this->repaginate_button($structure, $pageurl);
        $output .= $this->selectmultiple_button($structure);
        $output .= html_writer::end_tag('div');

        $output .= $this->total_marks($assignmentquesobj->get_assignmentques());
        ...
        if ($structure->can_be_edited()) {
            ...
            $this->page->requires->js_call_amd('mod_assignmentques/assignmentquesquestionbank', 'init', [
                $thiscontext->id
            ]);

            $this->page->requires->js_call_amd('mod_assignmentques/add_random_question', 'init', [
                $thiscontext->id,
                $pagevars['cat'],
    ...
    /**
     * Render any warnings that might be required about the state of the assignmentques,
     * e.g. if it has been attempted, or if the shuffle questions option is
     * turned on.
     *
     * @param structure $structure the assignmentques structure.
     * @return string HTML to output.
     */
    public function assignmentques_state_warnings(structure $structure) {
     ...
     * @return string HTML to output.
     */
    public function assignmentques_information(structure $structure) {
        list($currentstatus, $explanation) = $structure->get_dates_summary();

        $output = html_writer::span(
                    get_string('numquestionsx', 'assignmentques', $structure->get_question_count()),
                    'numberofquestions') . ' | ' .
                html_writer::span($currentstatus, 'assignmentquesopeningstatus',
    ...
    /**
     * Render the form for setting a assignmentques' overall grade
     *
     * @param structure $structure the assignmentques structure.
     * @param \moodle_url $pageurl the canonical URL of this page.
     * @return string HTML to output.
    ...
    public function maximum_grade_input($structure, \moodle_url $pageurl) {
        ...
        $output .= html_writer::start_tag('form', array('method' => 'post', 'action' => 'edit.php',
                'class' => 'assignmentquessavegradesform form-inline'));
        ...
        $output .= html_writer::empty_tag('input', array('type' => 'text', 'id' => 'inputmaxgrade',
                ...
                'value' => $structure->formatted_assignmentques_grade(),
                'class' => 'form-control'));
        $output .= html_writer::empty_tag('input', array('type' => 'submit', 'class' => 'btn btn-secondary ml-1',
                'name' => 'savechanges', 'value' => get_string('save', 'assignmentques')));
     ...
     * @param structure $structure the structure of the assignmentques being edited.
     * @param \moodle_url $pageurl the canonical URL of this page.
     * @return string HTML to output.
     */
    protected function repaginate_button(structure $structure, \moodle_url $pageurl) {
        $header = html_writer::tag('span', get_string('repaginatecommand', 'assignmentques'), array('class' => 'repaginatecommand'));
        ...
        $buttonoptions = array(
            ...
            'value' => get_string('repaginatecommand', 'assignmentques'),
        ...
        if (!$structure->can_be_repaginated()) {
            ...
            $this->page->requires->js_call_amd('mod_assignmentques/repaginate', 'init');
        }
    ...
    protected function selectmultiple_button(structure $structure) {
        $buttonoptions = array(
            ...
            'value' => get_string('selectmultipleitems', 'assignmentques'),
            'class' => 'btn btn-secondary mb-1'
        );
        if (!$structure->can_be_edited()) {
            $buttonoptions['disabled'] = 'disabled';
        ...
        return html_writer::tag('button', get_string('selectmultipleitems', 'assignmentques'), $buttonoptions);
    }
    ...
    protected function selectmultiple_controls(structure $structure) {
        ...
        $buttondeleteoptions = array(
            ...
            'value' => get_string('deleteselected', 'mod_assignmentques'),
        ...
        $output .= html_writer::tag('div',
                        html_writer::tag('button', get_string('deleteselected', 'mod_assignmentques'), $buttondeleteoptions) .
                        " " .
        ...
        $toolbaroptions = array(
            'class' => 'btn-toolbar',
            'role' => 'toolbar',
            'aria-label' => get_string('selectmultipletoolbar', 'assignmentques'),
        );
        ...
        $output .= html_writer::tag('div',
                html_writer::tag('div',
                        html_writer::link('#', get_string('selectall', 'assignmentques'), $buttonselectalloptions) .
                        html_writer::tag('span', "/", ['class' => 'separator']) .
                        html_writer::link('#', get_string('selectnone', 'assignmentques'), $buttondeselectalloptions),
    ...
    protected function repaginate_form(structure $structure, \moodle_url $pageurl) {
        $perpage = array();
        $perpage[0] = get_string('allinone', 'assignmentques');
        for ($i = 1; $i <= 50; ++$i) {
            $perpage[$i] = $i;
        }
        ...
        $formcontent = html_writer::tag('form', html_writer::div(
                    ...
                    get_string('repaginate', 'assignmentques', $select) .
    ...
    /**
     * Render the total marks available for the assignmentques.
     *
     * @param \stdClass $assignmentques the assignmentques settings from the database.
     * @return string HTML to output.
     */
    public function total_marks($assignmentques) {
        $totalmark = html_writer::span(assignmentques_format_grade($assignmentques, $assignmentques->sumgrades), 'mod_assignmentques_summarks');
        return html_writer::tag('span',
                get_string('totalmarksx', 'assignmentques', $totalmark),
     ...
     *
     * @param structure $structure the structure of the assignmentques being edited.
     * @param \stdClass $section The assignmentques_section entry from DB
     * @return string HTML to output.
     */
    protected function start_section($structure, $section) {
        ...
        if (!$structure->can_be_edited()) {
            ...
            $editsectionheadingicon = html_writer::link(new \moodle_url('#'),
                $this->pix_icon('t/editstring', get_string('sectionheadingedit', 'assignmentques', $section->heading),
     ...
     *
     * @param structure $structure object containing the structure of the assignmentques.
     * @param \stdClass $section data from the assignmentques_section table.
     * @return string HTML to output.
     */
    public function section_shuffle_questions(structure $structure, $section) {
        ...
        if ($structure->is_first_section($section)) {
            $help = $this->help_icon('shufflequestions', 'assignmentques');
        } else {
            $help = '';
        }
        ...
        $label = html_writer::label(get_string('shufflequestions', 'assignmentques'),
    ...
    /**
     * Render an icon to remove a section from the assignmentques.
     *
     * @param object $section the section to be removed.
     * @return string HTML to output.
     */
    public function section_remove_icon($section) {
        $title = get_string('sectionheadingremove', 'assignmentques', $section->heading);
        $url = new \moodle_url('/mod/assignmentques/edit.php',
    ...
    /**
     * Renders HTML to display the questions in a section of the assignmentques.
     *
     * This function calls {@link core_course_renderer::assignmentques_section_question()}
     *
     * @param structure $structure object containing the structure of the assignmentques.
    ...
    public function question_row(structure $structure, $slot, $contexts, $pagevars, $pageurl) {
        ...
        $joinhtml = '';
        if ($structure->can_be_edited() && !$structure->is_last_slot_in_assignmentques($slot) &&
    ...
    public function add_menu_actions(structure $structure, $page, \moodle_url $pageurl,
        ...
        $menu->set_constraint('.mod-assignmentques-edit-content');
        $trigger = html_writer::tag('span', get_string('add', 'assignmentques'), array('class' => 'add-menu'));
        ...
        // Disable the link if assignmentques has attempts.
        if (!$structure->can_be_edited()) {
    ...
    public function edit_menu_actions(structure $structure, $page,
        ...
        if (!isset($str)) {
            $str = get_strings(array('addasection', 'addaquestion', 'addarandomquestion',
                    'addarandomselectedquestion', 'questionbank'), 'assignmentques');
        }

        // Get section, page, slotnumber and maxmark.
        $actions = array();

        // Add a new question to the assignmentques.
        ...
        $params = array('returnurl' => $returnurl->out_as_local_url(false),
                ...
                'addonpage' => $page, 'appendqnumstring' => 'addquestion','courseid'=>$COURSE->id,'sesskey'=>$USER->sesskey,
            'qtype'=>'essay');
       ...
       /* $icon = new \pix_icon('t/add', $str->questionbank, 'moodle', array('class' => 'iconsmall', 'title' => ''));
        if ($page) {
            $title = get_string('addquestionfrombanktopage', 'assignmentques', $page);
        } else {
            $title = get_string('addquestionfrombankatend', 'assignmentques');
        }
        ...
        */
        ...
        /*if ($structure->can_add_random_questions()) {
            $returnurl = new \moodle_url('/mod/assignmentques/edit.php', array('cmid' => $structure->get_cmid(), 'data-addonpage' => $page));
            $params = ['returnurl' => $returnurl, 'cmid' => $structure->get_cmid(), 'appendqnumstring' => 'addarandomquestion'];
            $url = new \moodle_url('/mod/assignmentques/addrandom.php', $params);
            ...
            if ($page) {
                $title = get_string('addrandomquestiontopage', 'assignmentques', $page);
            } else {
                $title = get_string('addrandomquestionatend', 'assignmentques');
            }
    ...
    /**
     * Render the form that contains the data for adding a new question to the assignmentques.
     *
     * @param structure $structure object containing the structure of the assignmentques.
     * @param int $page the page number that this menu will add to.
     * @param \moodle_url $pageurl the canonical URL of this page.
    ...
    public function question(structure $structure, $slot, \moodle_url $pageurl) {
        ...
        // Action icons.
        $questionicons = '';
        $questionicons .= $this->question_preview_icon($structure->get_assignmentques(), $structure->get_question_in_slot($slot));
        if ($structure->can_be_edited()) {
     ...
     *
     * @param \stdClass $assignmentques the assignmentques settings from the database.
     * @param \stdClass $question data from the question and assignmentques_slots tables.
     * @param bool $label if true, show the preview question label after the icon
     * @param int $variant which question variant to preview (optional).
     * @return string HTML to output.
     */
    public function question_preview_icon($assignmentques, $question, $label = null, $variant = null) {
        $url = assignmentques_question_preview_url($assignmentques, $question, $variant);

        // Do we want a label?
        $strpreviewlabel = '';
        if ($label) {
            $strpreviewlabel = ' ' . get_string('preview', 'assignmentques');
        }

        // Build the icon.
        $strpreviewquestion = get_string('previewquestion', 'assignmentques');
    ...
    /**
     * Render an icon to remove a question from the assignmentques.
     *
     * @param structure $structure object containing the structure of the assignmentques.
     * @param int $slot the first slot on the page we are outputting.
    ...
    /**
     * Display an icon to split or join two pages of the assignmentques.
     *
     * @param structure $structure object containing the structure of the assignmentques.
     * @param int $slot the first slot on the page we are outputting.
     * @return string HTML to output.
    ...
    public function page_split_join_button($structure, $slot) {
        ...
        $url = new \moodle_url('repaginate.php', array('assignmentquesid' => $structure->get_assignmentquesid(),
                'slot' => $slot, 'repag' => $insertpagebreak ? 2 : 1, 'sesskey' => sesskey()));

        if ($insertpagebreak) {
            $title = get_string('addpagebreak', 'assignmentques');
            $image = $this->image_icon('e/insert_page_break', $title);
            $action = 'addpagebreak';
        } else {
            $title = get_string('removepagebreak', 'assignmentques');
    ...
    public function question_dependency_icon($structure, $slot) {
        ...
        if ($structure->is_question_dependent_on_previous_slot($slot)) {
            $title = get_string('questiondependencyremove', 'assignmentques', $a);
            $image = $this->pix_icon('t/locked', get_string('questiondependsonprevious', 'assignmentques'),
                    'moodle', array('title' => ''));
            $action = 'removedependency';
        } else {
            $title = get_string('questiondependencyadd', 'assignmentques', $a);
            $image = $this->pix_icon('t/unlocked', get_string('questiondependencyfree', 'assignmentques'),
                    'moodle', array('title' => ''));
    ...
    /**
     * Renders html to display a name with the link to the question on a assignmentques edit page
     *
    ...
    public function question_name(structure $structure, $slot, $pageurl) {
        ...
        $instancename = assignmentques_question_tostring($question);
        ...
        $output .= html_writer::link($editurl, $activitylink,
                array('title' => get_string('editquestion', 'assignmentques').' '.$title));
    ...
    public function random_question(structure $structure, $slotnumber, $pageurl) {
        ...
        $editurl = new \moodle_url('/mod/assignmentques/editrandom.php',
                array('returnurl' => $pageurl->out_as_local_url(), 'slotid' => $slot->id));

        $temp = clone($question);
        $temp->questiontext = '';
        $instancename = assignmentques_question_tostring($temp);

        $configuretitle = get_string('configurerandomquestion', 'assignmentques');
        $qtype = \question_bank::get_qtype($question->qtype, false);
        $namestr = $qtype->local_name();
        $icon = $this->pix_icon('icon', $namestr, $qtype->plugin_name(), array('title' => $namestr,
        ...
        $qbanklink = ' ' . \html_writer::link($qbankurl,
                get_string('seequestions', 'assignmentques'), array('class' => 'mod_assignmentques_random_qbank_link'));
    ...
    public function marked_out_of_field(structure $structure, $slot) {
        ...
        $output = html_writer::span($structure->formatted_question_grade($slot),
                'instancemaxmark decimalplaces_' . $structure->get_decimal_places_for_question_marks(),
                array('title' => get_string('maxmark', 'assignmentques')));
        ...
        $output .= html_writer::span(
            html_writer::link(
                ...
                array(
                    ...
                    'title' => get_string('editmaxmark', 'assignmentques'),
                )
            )
        );
    ...
    public function render_question_chooser(renderable $chooser) {
        return $this->render_from_template('mod_assignmentques/question_chooser', $chooser->export_for_template($this));
    }
    ...
    public function question_chooser() {
        $chooser = \mod_assignmentques\output\question_chooser::get($this->page->course, [], null);
    ...
    protected function initialise_editing_javascript(structure $structure,
        ...
        $config = new \stdClass();
        $config->resourceurl = '/mod/assignmentques/edit_rest.php';
        $config->sectionurl = '/mod/assignmentques/edit_rest.php';
        ...
        $this->page->requires->yui_module('moodle-mod_assignmentques-toolboxes',
                'M.mod_assignmentques.init_resource_toolbox',
                array(array(
                        'courseid' => $structure->get_courseid(),
                        'assignmentquesid' => $structure->get_assignmentquesid(),
        ...
        $this->page->requires->strings_for_js(array('areyousureremoveselected'), 'assignmentques');
        $this->page->requires->yui_module('moodle-mod_assignmentques-toolboxes',
                'M.mod_assignmentques.init_section_toolbox',
                array(array(
                        'courseid' => $structure,
                        'assignmentquesid' => $structure->get_assignmentquesid(),
        ...
        $this->page->requires->yui_module('moodle-mod_assignmentques-dragdrop', 'M.mod_assignmentques.init_section_dragdrop',
                array(array(
                        'courseid' => $structure,
                        'assignmentquesid' => $structure->get_assignmentquesid(),
                        'ajaxurl' => $config->sectionurl,
        ...
        $this->page->requires->yui_module('moodle-mod_assignmentques-dragdrop', 'M.mod_assignmentques.init_resource_dragdrop',
                array(array(
                        'courseid' => $structure,
                        'assignmentquesid' => $structure->get_assignmentquesid(),
                        'ajaxurl' => $config->resourceurl,
    ...
    protected function new_page_template(structure $structure,
        ...
        $strcontexts[] = 'addonpage="';
        $strcontexts[] = get_string('addquestionfrombanktopage', 'assignmentques', '');
     ...
     *
     * @param \mod_assignmentques\question\bank\custom_view $questionbank the question bank view object.
     * @param array $pagevars the variables from {@link \question_edit_setup()}.
     * @return string HTML to output / send back in response to an AJAX request.
     */
    public function question_bank_contents(\mod_assignmentques\question\bank\custom_view $questionbank, array $pagevars) {
