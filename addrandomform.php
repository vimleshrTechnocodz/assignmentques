...
/**
 * Defines the Moodle forum used to add random questions to the assignmentques.
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class assignmentques_add_random_form extends moodleform {
    ...
    protected function definition() {
        ...
        $mform->addElement('header', 'existingcategoryheader',
                get_string('randomfromexistingcategory', 'assignmentques'));
        ...
        $mform->addElement('checkbox', 'includesubcategories', '', get_string('recurse', 'assignmentques'));
        ...
        if ($CFG->usetags) {
            ...
            $options = array(
                'multiple' => true,
                'noselectionstring' => get_string('anytags', 'assignmentques'),
            );
            $mform->addElement('autocomplete', 'fromtags', get_string('randomquestiontags', 'mod_assignmentques'), $tagstrings, $options);
            $mform->addHelpButton('fromtags', 'randomquestiontags', 'mod_assignmentques');
        }

        $mform->addElement('select', 'numbertoadd', get_string('randomnumber', 'assignmentques'),
                $this->get_number_of_questions_to_add_choices());

        $previewhtml = $OUTPUT->render_from_template('mod_assignmentques/random_question_form_preview', []);
        $mform->addElement('html', $previewhtml);

        $mform->addElement('submit', 'existingcategory', get_string('addrandomquestion', 'assignmentques'));

        // Random from a new category section.
        $mform->addElement('header', 'newcategoryheader',
                get_string('randomquestionusinganewcategory', 'assignmentques'));
        ...
        $mform->addElement('submit', 'newcategory',
                get_string('createcategoryandaddrandomquestion', 'assignmentques'));
        ...
        $PAGE->requires->js_call_amd('mod_assignmentques/add_random_form', 'init', [
