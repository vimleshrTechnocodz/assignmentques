...
/**
 * Rest endpoint for ajax editing of assignmentques structure.
 *
...
$PAGE->set_url('/mod/assignmentques/edit-rest.php',
        array('assignmentquesid' => $assignmentquesid, 'class' => $class));
...
switch($requestmethod) {
    ...
    case 'GET': // For debugging.
        switch ($class) {
            case 'section':
                $table = 'assignmentques_sections';
                $section = $structure->get_section_by_id($id);
                switch ($field) {
                    case 'getsectiontitle':
                        require_capability('mod/assignmentques:manage', $modcontext);
            ...
            case 'resource':
                switch ($field) {
                    ...
                    case 'getmaxmark':
                        require_capability('mod/assignmentques:manage', $modcontext);
                        $slot = $DB->get_record('assignmentques_slots', array('id' => $id), '*', MUST_EXIST);
                        $result = array('instancemaxmark' => assignmentques_format_question_grade($assignmentques, $slot->maxmark));
                        break;
                    ...
                    case 'updatemaxmark':
                        ...
                        $result = array('instancemaxmark' => assignmentques_format_question_grade($assignmentques, $maxmark),
                                'newsummarks' => assignmentques_format_grade($assignmentques, $assignmentques->sumgrades));
                        break;
                    ...
                    case 'deletemultiple':
                        ...
                        $ids = explode(',', $ids);
                        foreach ($ids as $id) {
                            $slot = $DB->get_record('assignmentques_slots', array('assignmentquesid' => $assignmentques->id, 'id' => $id),
                                    '*', MUST_EXIST);
                            if (assignmentques_has_question_use($assignmentques, $slot->slot)) {
                        ...
                        $result = array('newsummarks' => assignmentques_format_grade($assignmentques, $assignmentques->sumgrades),
    ...
    case 'DELETE':
        switch ($class) {
            ...
            case 'resource':
                require_capability('mod/assignmentques:manage', $modcontext);
                if (!$slot = $DB->get_record('assignmentques_slots', array('assignmentquesid' => $assignmentques->id, 'id' => $id))) {
