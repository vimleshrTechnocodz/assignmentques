...
$cm = get_coursemodule_from_id('assignmentques', $id, 0, false, MUST_EXIST);
$course = $DB->get_record('course', array('id' => $cm->course), '*', MUST_EXIST);
$assignmentques = $DB->get_record('assignmentques', array('id' => $cm->instance), '*', MUST_EXIST);
require_login($course, false, $cm);

$reportlist = assignmentques_report_list(context_module::instance($cm->id));
if (empty($reportlist) || $userid == $USER->id) {
    ...
    // appear in the gradebook are unlikely to be able to see assignmentques reports,
    // but it is possible.)
    redirect(new moodle_url('/mod/assignmentques/view.php', array('id' => $cm->id)));
}
...
if ($userid) {
    ...
    $attempts = assignmentques_get_user_attempts($assignmentques->id, $userid, 'finished');
    $attempt = null;
    ...
    if ($attempt) {
        $attemptobj = new assignmentques_attempt($attempt, $assignmentques, $cm, $course, false);
...
redirect(new moodle_url('/mod/assignmentques/report.php', array(
