...
// Get the assignmentques slot.
$slot = $DB->get_record('assignmentques_slots', array('id' => $slotid));
if (!$slot || empty($slot->questioncategoryid)) {
    print_error('invalidrandomslot', 'mod_assignmentques');
}

if (!$assignmentques = $DB->get_record('assignmentques', array('id' => $slot->assignmentquesid))) {
    print_error('invalidassignmentquesid', 'assignmentques');
}

$cm = get_coursemodule_from_instance('assignmentques', $slot->assignmentquesid, $assignmentques->course);
...
if ($returnurl) {
    ...
    $returnurl = new moodle_url('/mod/assignmentques/edit.php', array('cmid' => $cm->id));
}

$url = new moodle_url('/mod/assignmentques/editrandom.php', array('slotid' => $slotid));
$PAGE->set_url($url);
...
$mform = new mod_assignmentques\form\randomquestion_form(new moodle_url('/mod/assignmentques/editrandom.php'),
...
$currentslottags = assignmentques_retrieve_slot_tags($slot->id);
...
if ($mform->is_cancelled()) {
    ...
    // We need to save some data into the assignmentques_slots table.
    ...
    foreach ($tags as $tag) {
        if ($key = array_search(['tagid' => $tag->id, 'tagname' => $tag->name], $searchableslottags)) {
            // If found, $key would be the id field in the assignmentques_slot_tags table.
    ...
    if (!empty($recordstokeep)) {
        ...
        $DB->delete_records_select('assignmentques_slot_tags', "slotid = ? AND id $select", $params);
    } else {
    ...
    if (!empty($recordstoinsert)) {
        $DB->insert_records('assignmentques_slot_tags', $recordstoinsert);
    }
...
echo $OUTPUT->header();
$heading = get_string('randomediting', 'mod_assignmentques');
echo $OUTPUT->heading_with_help($heading, 'randomquestion', 'mod_assignmentques');
