...
/**
 * Unit tests for the assignmentquesaccess_timelimit plugin.
 *
...
global $CFG;
require_once($CFG->dirroot . '/mod/assignmentques/accessrule/timelimit/rule.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class assignmentquesaccess_timelimit_testcase extends basic_testcase {
    public function test_time_limit_access_rule() {
        $assignmentques = new stdClass();
        $assignmentques->timeclose = 0;
        $assignmentques->timelimit = 3600;
        $cm = new stdClass();
        $cm->id = 0;
        $assignmentquesobj = new assignmentques($assignmentques, $cm, null);
        $rule = new assignmentquesaccess_timelimit($assignmentquesobj, 10000);
        $attempt = new stdClass();

        $this->assertEquals($rule->description(),
            get_string('assignmentquestimelimit', 'assignmentquesaccess_timelimit', format_time(3600)));
    ...
    /**
     * Test the time_left_display method of the assignmentquesaccess_timelimit class.
     *
     * @param int $timetoclose  The number of seconds that is left to the assignmentques' closing time
     * @param int $timelimit    Time limit of the assignmentques
     * @param int $displaylimit The limit that is displayed on the assignmentques page
    ...
    public function test_time_limit_access_rule_with_time_close($timetoclose, $timelimit, $displaylimit, $actuallimit) {
        ...
        $assignmentques = new stdClass();
        $assignmentques->timeclose = $timenow + $timetoclose;
        $assignmentques->timelimit = $timelimit;
        $cm = new stdClass();
        $cm->id = 0;
        $assignmentquesobj = new assignmentques($assignmentques, $cm, null);
        $rule = new assignmentquesaccess_timelimit($assignmentquesobj, $timenow);
        $attempt = new stdClass();

        $this->assertEquals($rule->description(),
            get_string('assignmentquestimelimit', 'assignmentquesaccess_timelimit', format_time($displaylimit)));
