...
/**
 * Steps definitions related to mod_assignmentques.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class behat_mod_assignmentques extends behat_question_base {
    ...
    protected function resolve_page_url(string $page): moodle_url {
        switch ($page) {
            default:
                throw new Exception('Unrecognised assignmentques page type "' . $page . '."');
        }
    }
     ...
     * | View              | Assignmentques name                                   | The assignmentques info page (view.php)                |
     * | Edit              | Assignmentques name                                   | The edit assignmentques page (edit.php)                |
     * | Group overrides   | Assignmentques name                                   | The manage group overrides page              |
     * | User overrides    | Assignmentques name                                   | The manage user overrides page               |
     * | Grades report     | Assignmentques name                                   | The overview report for a assignmentques               |
     * | Responses report  | Assignmentques name                                   | The responses report for a assignmentques              |
     * | Statistics report | Assignmentques name                                   | The statistics report for a assignmentques             |
     * | Attempt review    | Assignmentques name > username > [Attempt] attempt no | Review page for a given attempt (review.php) |
     *
     * @param string $type identifies which type of page this is, e.g. 'Attempt review'.
     * @param string $identifier identifies the particular page, e.g. 'Test assignmentques > student > Attempt 1'.
     * @return moodle_url the corresponding URL.
     * @throws Exception with a meaningful error message if the specified page cannot be found.
     */
    protected function resolve_page_instance_url(string $type, string $identifier): moodle_url {
        ...
        switch ($type) {
            case 'View':
                return new moodle_url('/mod/assignmentques/view.php',
                        ['id' => $this->get_cm_by_assignmentques_name($identifier)->id]);

            case 'Edit':
                return new moodle_url('/mod/assignmentques/edit.php',
                        ['cmid' => $this->get_cm_by_assignmentques_name($identifier)->id]);

            case 'Group overrides':
                return new moodle_url('/mod/assignmentques/overrides.php',
                    ['cmid' => $this->get_cm_by_assignmentques_name($identifier)->id, 'mode' => 'group']);

            case 'User overrides':
                return new moodle_url('/mod/assignmentques/overrides.php',
                    ['cmid' => $this->get_cm_by_assignmentques_name($identifier)->id, 'mode' => 'user']);

            case 'Grades report':
                return new moodle_url('/mod/assignmentques/report.php',
                    ['id' => $this->get_cm_by_assignmentques_name($identifier)->id, 'mode' => 'overview']);

            case 'Responses report':
                return new moodle_url('/mod/assignmentques/report.php',
                    ['id' => $this->get_cm_by_assignmentques_name($identifier)->id, 'mode' => 'responses']);

            case 'Statistics report':
                return new moodle_url('/mod/assignmentques/report.php',
                    ['id' => $this->get_cm_by_assignmentques_name($identifier)->id, 'mode' => 'statistics']);

            case 'Attempt review':
                if (substr_count($identifier, ' > ') !== 2) {
                    throw new coding_exception('For "attempt review", name must be ' .
                            '"{Assignmentques name} > {username} > Attempt {attemptnumber}", ' .
                            'for example "Assignmentques 1 > student > Attempt 1".');
                }
                list($assignmentquesname, $username, $attemptno) = explode(' > ', $identifier);
                $attemptno = (int) trim(str_replace ('Attempt', '', $attemptno));
                $assignmentques = $this->get_assignmentques_by_name($assignmentquesname);
                $user = $DB->get_record('user', ['username' => $username], '*', MUST_EXIST);
                $attempt = $DB->get_record('assignmentques_attempts',
                        ['assignmentques' => $assignmentques->id, 'userid' => $user->id, 'attempt' => $attemptno], '*', MUST_EXIST);
                return new moodle_url('/mod/assignmentques/review.php', ['attempt' => $attempt->id]);

            default:
                throw new Exception('Unrecognised assignmentques page type "' . $type . '."');
        }
    }

    /**
     * Get a assignmentques by name.
     *
     * @param string $name assignmentques name.
     * @return stdClass the corresponding DB row.
     */
    protected function get_assignmentques_by_name(string $name): stdClass {
        global $DB;
        return $DB->get_record('assignmentques', array('name' => $name), '*', MUST_EXIST);
    }

    /**
     * Get a assignmentques cmid from the assignmentques name.
     *
     * @param string $name assignmentques name.
     * @return stdClass cm from get_coursemodule_from_instance.
     */
    protected function get_cm_by_assignmentques_name(string $name): stdClass {
        $assignmentques = $this->get_assignmentques_by_name($name);
        return get_coursemodule_from_instance('assignmentques', $assignmentques->id, $assignmentques->course);
    }

    /**
     * Put the specified questions on the specified pages of a given assignmentques.
     *
     ...
     *
     * @param string $assignmentquesname the name of the assignmentques to add questions to.
     * @param TableNode $data information about the questions to add.
     *
     * @Given /^assignmentques "([^"]*)" contains the following questions:$/
     */
    public function assignmentques_contains_the_following_questions($assignmentquesname, TableNode $data) {
        global $DB;
        ...
        if (!in_array('question', $firstrow) && !in_array('page', $firstrow)) {
            if (count($firstrow) == 2) {
                ...
                throw new ExpectationException('When adding questions to a assignmentques, you should give 2 or three 3 things: ' .
        ...
        foreach ($data->getHash() as $questiondata) {
            if (!array_key_exists('question', $questiondata)) {
                throw new ExpectationException('When adding questions to a assignmentques, ' .
            ...
            if ($question->qtype == 'random') {
                ...
                assignmentques_add_random_questions($assignmentques, $page, $question->category, 1, $includingsubcategories);
            } else {
                // Add the question.
                assignmentques_add_assignmentques_question($question->id, $assignmentques, $page, $maxmark);
            }

            // Require previous.
            if (array_key_exists('requireprevious', $questiondata)) {
                if ($questiondata['requireprevious'] === '1') {
                    $slot = $DB->get_field('assignmentques_slots', 'MAX(slot)', array('assignmentquesid' => $assignmentques->id));
                    $DB->set_field('assignmentques_slots', 'requireprevious', 1,
                            array('assignmentquesid' => $assignmentques->id, 'slot' => $slot));
        ...
        assignmentques_update_sumgrades($assignmentques);
    }

    /**
     * Put the specified section headings to start at specified pages of a given assignmentques.
     *
     * The first row should be column names:
     ...
     *
     * @param string $assignmentquesname the name of the assignmentques to add sections to.
     * @param TableNode $data information about the sections to add.
     *
     * @Given /^assignmentques "([^"]*)" contains the following sections:$/
     */
    public function assignmentques_contains_the_following_sections($assignmentquesname, TableNode $data) {
        global $DB;

        $assignmentques = $DB->get_record('assignmentques', array('name' => $assignmentquesname), '*', MUST_EXIST);

        // Add the sections.
        $previousfirstslot = 0;
        foreach ($data->getHash() as $rownumber => $sectiondata) {
            if (!array_key_exists('heading', $sectiondata)) {
                throw new ExpectationException('When adding sections to a assignmentques, ' .
            ...
            if ($rownumber == 0) {
                $section = $DB->get_record('assignmentques_sections', array('assignmentquesid' => $assignmentques->id), '*', MUST_EXIST);
            } else {
                $section = new stdClass();
                $section->assignmentquesid = $assignmentques->id;
            }
            ...
            if ($rownumber == 0) {
                ...
                $DB->insert_record('assignmentques_sections', $section);
            }
        }

        if ($section->firstslot > $DB->count_records('assignmentques_slots', array('assignmentquesid' => $assignmentques->id))) {
            throw new ExpectationException('The section firstslot must be less than the total number of slots in the assignmentques.',
                    $this->getSession());
        }
    }

    /**
     * Adds a question to the existing assignmentques with filling the form.
     *
     * The form for creating a question should be on one page.
     *
     * @When /^I add a "(?P<question_type_string>(?:[^"]|\\")*)" question to the "(?P<assignmentques_name_string>(?:[^"]|\\")*)" assignmentques with:$/
     * @param string $questiontype
     * @param string $assignmentquesname
     * @param TableNode $questiondata with data for filling the add question form
     */
    public function i_add_question_to_the_assignmentques_with($questiontype, $assignmentquesname, TableNode $questiondata) {
        $assignmentquesname = $this->escape($assignmentquesname);
        $editassignmentques = $this->escape(get_string('editassignmentques', 'assignmentques'));
        $assignmentquesadmin = $this->escape(get_string('pluginadministration', 'assignmentques'));
        $addaquestion = $this->escape(get_string('addaquestion', 'assignmentques'));

        $this->execute('behat_general::click_link', $assignmentquesname);

        $this->execute("behat_navigation::i_navigate_to_in_current_page_administration",
                $assignmentquesadmin . ' > ' . $editassignmentques);
    ...
    /**
     * Set the max mark for a question on the Edit assignmentques page.
     *
     ...
     */
    public function i_set_the_max_mark_for_assignmentques_question($questionname, $newmark) {
        $this->execute('behat_general::click_link', $this->escape(get_string('editmaxmark', 'assignmentques')));
    ...
    /**
     * Open the add menu on a given page, or at the end of the Edit assignmentques page.
     * @Given /^I open the "(?P<page_n_or_last_string>(?:[^"]|\\")*)" add to assignmentques menu$/
     * @param string $pageorlast either "Page n" or "last".
     */
    public function i_open_the_add_to_assignmentques_menu_for($pageorlast) {
        ...
        if ($pageorlast == 'last') {
            ...
            throw new ExpectationException("The I open the add to assignmentques menu step must specify either 'Page N' or 'last'.");
        }
        $this->find('xpath', $xpath)->click();
    }

    /**
     * Check whether a particular question is on a particular page of the assignmentques on the Edit assignmentques page.
     * @Given /^I should see "(?P<question_name>(?:[^"]|\\")*)" on assignmentques page "(?P<page_number>\d+)"$/
     ...
     */
    public function i_should_see_on_assignmentques_page($questionname, $pagenumber) {
    ...
    /**
     * Check whether a particular question is not on a particular page of the assignmentques on the Edit assignmentques page.
     * @Given /^I should not see "(?P<question_name>(?:[^"]|\\")*)" on assignmentques page "(?P<page_number>\d+)"$/
     * @param string $questionname the name of the question we are looking for.
     * @param number $pagenumber the page it should be found on.
     */
    public function i_should_not_see_on_assignmentques_page($questionname, $pagenumber) {
        $xpath = "//li[contains(., '" . $this->escape($questionname) .
                "')][./preceding-sibling::li[contains(@class, 'pagenumber')][1][contains(., 'Page " .
                $pagenumber . "')]]";
    ...
    /**
     * Check whether one question comes before another on the Edit assignmentques page.
     * The two questions must be on the same page.
     * @Given /^I should see "(?P<first_q_name>(?:[^"]|\\")*)" before "(?P<second_q_name>(?:[^"]|\\")*)" on the edit assignmentques page$/
     ...
     */
    public function i_should_see_before_on_the_edit_assignmentques_page($firstquestionname, $secondquestionname) {
    ...
    /**
     * Check the number displayed alongside a question on the Edit assignmentques page.
     * @Given /^"(?P<question_name>(?:[^"]|\\")*)" should have number "(?P<number>(?:[^"]|\\")*)" on the edit assignmentques page$/
     * @param string $questionname the name of the question we are looking for.
     * @param number $number the number (or 'i') that should be displayed beside that question.
     */
    public function should_have_number_on_the_edit_assignmentques_page($questionname, $number) {
     ...
     *
     * @Given /^I click on shuffle for section "([^"]*)" on the assignmentques edit page$/
     */
     ...
     *
     * @Given /^shuffle for section "([^"]*)" should be "(On|Off)" on the assignmentques edit page$/
     */
    public function shuffle_for_section_should_be($heading, $value) {
        ...
        if ($value == 'On' && !$checkbox->isChecked()) {
            $msg = "Shuffle for section '$heading' is not checked, but you are expecting it to be checked ($value). " .
                    "Check the line with: \nshuffle for section \"$heading\" should be \"$value\" on the assignmentques edit page" .
    ...
    /**
     * Move a question on the Edit assignmentques page by first clicking on the Move icon,
     * then clicking one of the "After ..." links.
     * @When /^I move "(?P<question_name>(?:[^"]|\\")*)" to "(?P<target>(?:[^"]|\\")*)" in the assignmentques by clicking the move icon$/
     * @param string $questionname the name of the question we are looking for.
    ...
    /**
     * Move a question on the Edit assignmentques page by dragging a given question on top of another item.
     * @When /^I move "(?P<question_name>(?:[^"]|\\")*)" to "(?P<target>(?:[^"]|\\")*)" in the assignmentques by dragging$/
     * @param string $questionname the name of the question we are looking for.
    ...
    /**
     * Delete a question on the Edit assignmentques page by first clicking on the Delete icon,
     * then clicking one of the "After ..." links.
     * @When /^I delete "(?P<question_name>(?:[^"]|\\")*)" in the assignmentques by clicking the delete icon$/
     * @param string $questionname the name of the question we are looking for.
     * @return array of steps.
     */
    ...
    /**
     * Set the section heading for a given section on the Edit assignmentques page
     *
     * @When /^I change assignmentques section heading "(?P<section_name_string>(?:[^"]|\\")*)" to "(?P<new_section_heading_string>(?:[^"]|\\")*)"$/
     ...
     * assignmentques navigation block.
     *
     * @Then /^I should see question "(?P<questionnumber>\d+)" in section "(?P<section_heading_string>(?:[^"]|\\")*)" in the assignmentques navigation$/
     ...
     */
    public function i_should_see_question_in_section_in_the_assignmentques_navigation($questionnumber, $sectionheading) {
        ...
        $exception = new ExpectationException('Question "' . $questionnumber . '" is not in section "' .
                $sectionheading . '" in the assignmentques navigation.', $this->getSession());
        $xpath = "//*[@id = 'mod_assignmentques_navblock']//*[contains(concat(' ', normalize-space(@class), ' '), ' qnbutton ') and " .
     ...
     * user_has_started_an_attempt_at_assignmentques_with_details, etc.
     *
     ...
     *      that can be passed to $assignmentquesgenerator->create_attempt.
     */
    protected function extract_forced_randomisation_from_attempt_info(TableNode $attemptinfo) {
        ...
        foreach ($attemptinfo->getHash() as $slotinfo) {
            if (empty($slotinfo['slot'])) {
                throw new ExpectationException('When simulating a assignmentques attempt, ' .
    ...
    /**
     * Helper used by user_has_attempted_with_responses, user_has_checked_answers_in_their_attempt_at_assignmentques,
     * user_has_input_answers_in_their_attempt_at_assignmentques, etc.
     *
     * @param TableNode $attemptinfo data table from the Behat step
     * @return array of responses that can be passed to $assignmentquesgenerator->submit_responses.
     */
     ...
     *
     * slot           The slot
     * actualquestion This column is optional, and is only needed if the assignmentques contains
     ...
     *                made when the assignmentques is started.
     ...
     * @param string $assignmentquesname the name of the assignmentques the user will attempt.
     * @param TableNode $attemptinfo information about the questions to add, as above.
     * @Given /^user "([^"]*)" has attempted "([^"]*)" with responses:$/
     */
    public function user_has_attempted_with_responses($username, $assignmentquesname, TableNode $attemptinfo) {
        global $DB;

        /** @var mod_assignmentques_generator $assignmentquesgenerator */
        $assignmentquesgenerator = behat_util::get_data_generator()->get_plugin_generator('mod_assignmentques');

        $assignmentquesid = $DB->get_field('assignmentques', 'id', ['name' => $assignmentquesname], MUST_EXIST);
        $user = $DB->get_record('user', ['username' => $username], '*', MUST_EXIST);
        ...
        $attempt = $assignmentquesgenerator->create_attempt($assignmentquesid, $user->id,
                $forcedrandomquestions, $forcedvariants);

        $assignmentquesgenerator->submit_responses($attempt->id, $responses, false, true);

        $this->set_user();
    }

    /**
     * Start a assignmentques attempt without answers.
     *
     * Then there should be a number of rows of data, one for each question you want to add.
     * There is no need to supply answers to all questions. If so, other qusetions will be
     * left unanswered.
     ...
     * @param string $username the username of the user that will attempt.
     * @param string $assignmentquesname the name of the assignmentques the user will attempt.
     * @Given /^user "([^"]*)" has started an attempt at assignmentques "([^"]*)"$/
     */
    public function user_has_started_an_attempt_at_assignmentques($username, $assignmentquesname) {
        global $DB;
        ...
        $user = $DB->get_record('user', ['username' => $username], '*', MUST_EXIST);
        $this->set_user($user);
        $assignmentquesgenerator->create_attempt($assignmentquesid, $user->id);
        $this->set_user();
    }
     ...
     * @param string $assignmentquesname the name of the assignmentques the user will attempt.
     * @param TableNode $attemptinfo information about the questions to add, as above.
     * @Given /^user "([^"]*)" has started an attempt at assignmentques "([^"]*)" randomised as follows:$/
     */
    public function user_has_started_an_attempt_at_assignmentques_with_details($username, $assignmentquesname, TableNode $attemptinfo) {
        global $DB;
    ...
    /**
     * Input answers to particular questions an existing assignmentques attempt, without
     ...
     * @Given /^user "([^"]*)" has input answers in their attempt at assignmentques "([^"]*)":$/
     */
    public function user_has_input_answers_in_their_attempt_at_assignmentques($username, $assignmentquesname, TableNode $attemptinfo) {
        global $DB;
        ...
        $attempts = assignmentques_get_user_attempts($assignmentquesid, $user->id, 'unfinished', true);
        $assignmentquesgenerator->submit_responses(key($attempts), $responses, false, false);

        $this->set_user();
    }

    /**
     * Submit answers to questions an existing assignmentques attempt, with a simulated click on the 'Check' button.
     *
     ...
     * @param TableNode $attemptinfo information about the questions to add, as above.
     * @throws \Behat\Mink\Exception\ExpectationException
     * @Given /^user "([^"]*)" has checked answers in their attempt at assignmentques "([^"]*)":$/
     */
    public function user_has_checked_answers_in_their_attempt_at_assignmentques($username, $assignmentquesname, TableNode $attemptinfo) {
        global $DB;
        ...
        $attempts = assignmentques_get_user_attempts($assignmentquesid, $user->id, 'unfinished', true);
        $assignmentquesgenerator->submit_responses(key($attempts), $responses, true, false);

        $this->set_user();
    }

    /**
     * Finish an existing assignmentques attempt.
     *
     * @param string $username the username of the user that will attempt.
     * @param string $assignmentquesname the name of the assignmentques the user will attempt.
     * @Given /^user "([^"]*)" has finished an attempt at assignmentques "([^"]*)"$/
     */
    public function user_has_finished_an_attempt_at_assignmentques($username, $assignmentquesname) {
        global $DB;
        ...
        $attempts = assignmentques_get_user_attempts($assignmentquesid, $user->id, 'unfinished', true);
        $attemptobj = assignmentques_attempt::create(key($attempts));
