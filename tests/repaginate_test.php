...
/**
 * Unit tests for the {@link \mod_assignmentques\repaginate} class.
...
require_once($CFG->dirroot . '/mod/assignmentques/classes/repaginate.php');


/**
 * Testable subclass, giving access to the protected methods of {@link \mod_assignmentques\repaginate}
 * @copyright 2014 The Open Univsersity
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_assignmentques_repaginate_testable extends \mod_assignmentques\repaginate {

    public function __construct($assignmentquesid = 0, $slots = null) {
        return parent::__construct($assignmentquesid, $slots);
    }
 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_assignmentques_repaginate_test extends advanced_testcase {

    /** @var array stores the slots. */
    private $assignmentquesslots;
    /** @var mod_assignmentques_repaginate_testable the object being tested. */
    private $repaginate = null;

    public function setUp() {
        $this->set_assignmentques_slots($this->get_assignmentques_object()->get_slots());
        $this->repaginate = new mod_assignmentques_repaginate_testable(0, $this->assignmentquesslots);
    }
    ...
    /**
     * Create a assignmentques, add five questions to the assignmentques
     * which are all on one page and return the assignmentques object.
     */
    private function get_assignmentques_object() {
        global $SITE;
        ...
        $assignmentques = $assignmentquesgenerator->create_instance(array(
                'course' => $SITE->id, 'questionsperpage' => 0, 'grade' => 100.0, 'sumgrades' => 2));
        $cm = get_coursemodule_from_instance('assignmentques', $assignmentques->id, $SITE->id);
        ...
        assignmentques_add_assignmentques_question($shortanswer->id, $assignmentques);
        assignmentques_add_assignmentques_question($numerical->id, $assignmentques);
        assignmentques_add_assignmentques_question($essay->id, $assignmentques);
        assignmentques_add_assignmentques_question($truefalse->id, $assignmentques);
        assignmentques_add_assignmentques_question($match->id, $assignmentques);

        // Return the assignmentques object.
        $assignmentquesobj = new assignmentques($assignmentques, $cm, $SITE);
        return \mod_assignmentques\structure::create_for_assignmentques($assignmentquesobj);
    }

    /**
     * Set the assignmentques slots
     * @param string $slots
     */
    private function set_assignmentques_slots($slots = null) {
        if (!$slots) {
            $this->assignmentquesslots = $this->get_assignmentques_object()->get_slots();
        } else {
            $this->assignmentquesslots = $slots;
        }
    }
    ...
    public function test_get_this_slot() {
        $this->set_assignmentques_slots();
        $actual = array();
        $expected = $this->repaginate->get_slots_by_slot_number();
        $this->assertEquals($expected, $actual);

        $slotsbyno = $this->repaginate->get_slots_by_slot_number($this->assignmentquesslots);
        $slotnumber = 5;
        $thisslot = $this->repaginate->get_this_slot($this->assignmentquesslots, $slotnumber);
    ...
    public function test_get_slots_by_slotnumber() {
        ...
        foreach ($this->assignmentquesslots as $slot) {
            $expected[$slot->slot] = $slot;
        }
        $actual = $this->repaginate->get_slots_by_slot_number($this->assignmentquesslots);
        $this->assertEquals($expected, $actual);
    }
    ...
    public function test_get_slots_by_slotid() {
        ...
        $this->assertEquals($this->assignmentquesslots, $actual);
    }
    ...
    public function test_repaginate_n_questions_per_page() {
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->assignmentquesslots, 2);
        $this->assertEquals($expected, $actual);
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->assignmentquesslots, 3);
        $this->assertEquals($expected, $actual);
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->assignmentquesslots, 5);
        $this->assertEquals($expected, $actual);
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->assignmentquesslots, 10);
        $this->assertEquals($expected, $actual);
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->assignmentquesslots, 1);
        $this->assertEquals($expected, $actual);
    }

    public function test_repaginate_this_slot() {
        $this->set_assignmentques_slots();
        $slotsbyslotno = $this->repaginate->get_slots_by_slot_number($this->assignmentquesslots);
        $slotnumber = 3;
        $newpagenumber = 2;
    ...
    public function test_repaginate_the_rest() {
        $this->set_assignmentques_slots();
        $slotfrom = 1;
        $type = \mod_assignmentques\repaginate::LINK;
        $expected = array();
        foreach ($this->assignmentquesslots as $slot) {
        ...
        $actual = $this->repaginate->repaginate_the_rest($this->assignmentquesslots, $slotfrom, $type, false);
        $this->assertEquals($expected, $actual);

        $slotfrom = 2;
        $newslots = array();
        foreach ($this->assignmentquesslots as $s) {
            if ($s->slot === $slotfrom) {
                $s->page = $s->page - 1;
            }
            $newslots[$s->id] = $s;
        ...
        $type = \mod_assignmentques\repaginate::UNLINK;
        $expected = array();
        foreach ($this->assignmentquesslots as $slot) {
