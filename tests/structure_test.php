 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_assignmentques_structure_testcase extends advanced_testcase {

    /**
     * Create a course with an empty assignmentques.
     * @return array with three elements assignmentques, cm and course.
     */
    protected function prepare_assignmentques_data() {
        ...
        return array($assignmentques, $cm, $course);
    }

    /**
     * Creat a test assignmentques.
     *
     ...
     * @return assignmentques the created assignmentques.
     */
    protected function create_test_assignmentques($layout) {
        list($assignmentques, $cm, $course) = $this->prepare_assignmentques_data();
        ...
        foreach ($layout as $item) {
            if (is_string($item)) {
                ...
                assignmentques_add_assignmentques_question($q->id, $assignmentques, $page);
                $lastpage = $page;
            }
        }

        $assignmentquesobj = new assignmentques($assignmentques, $cm, $course);
        $structure = \mod_assignmentques\structure::create_for_assignmentques($assignmentquesobj);
        if (isset($headings[1])) {
        ...
        return $assignmentquesobj;
    }

    /**
     * Verify that the given layout matches that expected.
     * @param array $expectedlayout as for $layout in {@link create_test_assignmentques()}.
     * @param \mod_assignmentques\structure $structure the structure to test.
     */
    protected function assert_assignmentques_layout($expectedlayout, \mod_assignmentques\structure $structure) {
        $sections = $structure->get_sections();
        ...
        foreach ($expectedlayout as $item) {
            if (is_string($item)) {
                ...
                if ($slot > 1 && $section->heading == '' && $section->firstslot == 1) {
                    // The array $expectedlayout did not contain default first assignmentques section, so skip over it.
                    $section = array_shift($sections);
                }
        ...
        if ($slot - 1 != count($structure->get_slots())) {
            $this->fail('The assignmentques contains more slots than expected.');
        }

        if (!empty($sections)) {
            $section = array_shift($sections);
            if ($section->heading != '' || $section->firstslot != 1) {
                $this->fail('Unexpected section (' . $section->heading .') found in the assignmentques.');
            }
        }
    }
     ...
     * used by create_test_assignmentques as assert_assignmentques_layout.
    ...
    public function test_get_assignmentques_slots() {
        $assignmentquesobj = $this->create_test_assignmentques(array(
                array('TF1', 1, 'truefalse'),
    ...
    public function test_assignmentques_has_one_section_by_default() {
        $assignmentquesobj = $this->create_test_assignmentques(array(
                array('TF1', 1, 'truefalse'),
            ));
        $structure = \mod_assignmentques\structure::create_for_assignmentques($assignmentquesobj);
    ...
    public function test_remove_section_heading() {
        ...
        $structure = \mod_assignmentques\structure::create_for_assignmentques($assignmentquesobj);
        $this->assert_assignmentques_layout(array(
                'Heading 1',
                array('TF1', 1, 'truefalse'),
                array('TF2', 2, 'truefalse'),
            ), $structure);
    ...
    public function test_assignmentques_remove_slot() {
        $assignmentquesobj = $this->create_test_assignmentques(array(
                array('TF1', 1, 'truefalse'),
                array('TF2', 1, 'truefalse'),
                'Heading 2',
    ...
    public function test_assignmentques_removing_a_random_question_deletes_the_question() {
        global $DB;
        ...
        $questiongenerator = $this->getDataGenerator()->get_plugin_generator('core_question');
        $cat = $questiongenerator->create_question_category();
        assignmentques_add_random_questions($assignmentquesobj->get_assignmentques(), 1, $cat->id, 1, false);
        $structure = \mod_assignmentques\structure::create_for_assignmentques($assignmentquesobj);
    ...
    public function test_can_remove_last_question_in_a_assignmentques() {
        $assignmentquesobj = $this->create_test_assignmentques(array(
                'Heading 1',
                array('TF1', 1, 'truefalse'),
            ));
        ...
        assignmentques_add_assignmentques_question($q->id, $assignmentquesobj->get_assignmentques(), 0);
        $structure = \mod_assignmentques\structure::create_for_assignmentques($assignmentquesobj);
    ...
    public function test_add_question_updates_headings() {
        ...
        assignmentques_add_assignmentques_question($q->id, $assignmentquesobj->get_assignmentques(), 1);
    ...
    public function test_remove_page_break() {
        ...
        $slots = $structure->update_page_break($slotid, \mod_assignmentques\repaginate::LINK);
    ...
    public function test_add_page_break() {
        ...
        $slotid = $structure->get_question_in_slot(2)->slotid;
        $slots = $structure->update_page_break($slotid, \mod_assignmentques\repaginate::UNLINK);
     ...
     * @param  array $layout Assignmentques layout for create_test_assignmentques function
    ...
    public function test_get_slot_tags_for_slot($layout, $tagnames, $slotnumber, $expected) {
        ...
        $assignmentques = $this->create_test_assignmentques($layout);
        $structure = \mod_assignmentques\structure::create_for_assignmentques($assignmentques);
        ...
        $DB->insert_records('assignmentques_slot_tags', $slottagrecords);
    ...
    public function test_can_add_random_questions() {
        ...
        $assignmentques = $this->create_test_assignmentques([]);
        $course = $assignmentques->get_course();
