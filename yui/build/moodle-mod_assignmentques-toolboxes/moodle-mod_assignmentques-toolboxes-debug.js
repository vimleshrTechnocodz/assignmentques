YUI.add('moodle-mod_assignmentques-toolboxes', function (Y, NAME) {
...
var CSS = {
    ...
    SELECTOR = {
        ...
        SHOW: 'a.' + CSS.SHOW,
        SLOTLI: 'li.slot',
        SUMMARKS: '.mod_assignmentques_summarks'
    },
...
Y.extend(TOOLBOX, Y.Base, {
    ...
    send_request: function(data, statusspinner, success_callback, optionalconfig) {
        ...
        data.assignmentquesid = this.get('assignmentquesid');
                ...
                success: function(tid, response) {
                    ...
                    if (responsetext.hasOwnProperty('newnumquestions')) {
                        Y.one(SELECTOR.NUMQUESTIONS).setHTML(
                                M.util.get_string('numquestionsx', 'assignmentques', responsetext.newnumquestions)
                            );
                    }
                    if (success_callback) {
...
{
    NAME: 'mod_assignmentques-toolbox',
    ATTRS: {
        /**
 ...
 *
 * This class is responsible for managing AJAX interactions with activities and resources
 * when viewing a assignmentques in editing mode.
 *
 * @module mod_assignmentques-resource-toolbox
 * @namespace M.mod_assignmentques.resource_toolbox
 */
...
Y.extend(RESOURCETOOLBOX, TOOLBOX, {
    ...
    initializer: function() {
        //M.mod_assignmentques.assignmentquesbase.register_module(this);//hello
    ...
    delete_with_confirmation: function(ev, button, activity) {
        ...
        confirmstring = M.util.get_string('confirmremovequestion', 'assignmentques', qtypename);
        ...
        confirm.on('complete-yes', function() {
            ...
            var data = {
                'class': 'resource',
                'action': 'DELETE',
                'id': Y.Moodle.mod_assignmentques.util.slot.getId(element)
            };
            this.send_request(data, spinner, function(response) {
                if (response.deleted) {
                    // Actually remove the element.
                    Y.Moodle.mod_assignmentques.util.slot.remove(element);
    ...
    delete_multiple_action: function(ev) {
        ...
        if (typeof problemsection !== 'undefined') {
            var alert = new M.core.alert({
                title: M.util.get_string('cannotremoveslots', 'assignmentques'),
                message: M.util.get_string('cannotremoveallsectionslots', 'assignmentques', problemsection)
            });
    ...
    delete_multiple_with_confirmation: function(ev) {
        ...
        Y.all(SELECTOR.SELECTMULTIPLECHECKBOX + ':checked').each(function(node) {
            var slot = Y.Moodle.mod_assignmentques.util.slot.getSlotFromComponent(node);
            ids += ids === '' ? '' : ',';
            ids += Y.Moodle.mod_assignmentques.util.slot.getId(slot);
            slots.push(slot);
        });
        var element = Y.one('div.mod-assignmentques-edit-content');
        ...
        // Create the confirmation dialogue.
        var confirm = new M.core.confirm({
            question: M.util.get_string('areyousureremoveselected', 'assignmentques'),
            modal: true
        });
        ...
        confirm.on('complete-yes', function() {
            ...
            this.send_request(data, spinner, function(response) {
                ...
                if (response.deleted) {
                    // Actually remove the element.
                    Y.all(SELECTOR.SELECTMULTIPLECHECKBOX + ':checked').each(function(node) {
                        Y.Moodle.mod_assignmentques.util.slot.remove(node.ancestor('li.activity'));
                    });
    ...
    edit_maxmark: function(ev, button, activity) {
        ...
        var instancemaxmark = activity.one(SELECTOR.INSTANCEMAXMARK),
            ...
            data = {
                'class': 'resource',
                'field': 'getmaxmark',
                'id': Y.Moodle.mod_assignmentques.util.slot.getId(activity)
            };
    ...
    update_page_break: function(ev, button, activity, action) {
        ...
        var data = {
            ...
            'id':    Y.Moodle.mod_assignmentques.util.slot.getId(nextactivity),
            'value': value
        };
        ...
        this.send_request(data, spinner, function(response) {
            if (response.slots) {
                if (action === 'addpagebreak') {
                    Y.Moodle.mod_assignmentques.util.page.add(activity);
                } else {
                    var page = activity.next(Y.Moodle.mod_assignmentques.util.page.SELECTORS.PAGE);
                    Y.Moodle.mod_assignmentques.util.page.remove(page, true);
                }
                this.reorganise_edit_page();
            }
        });
    ...
    update_dependency: function(ev, button, activity, action) {
        ...
        this.send_request(data, spinner, function(response) {
            if (response.hasOwnProperty('requireprevious')) {
                Y.Moodle.mod_assignmentques.util.slot.updateDependencyIcon(activity, response.requireprevious);
            }
        });
    ...
    reorganise_edit_page: function() {
        Y.Moodle.mod_assignmentques.util.slot.reorderSlots();
        Y.Moodle.mod_assignmentques.util.slot.reorderPageBreaks();
        Y.Moodle.mod_assignmentques.util.page.reorderPages();
        Y.Moodle.mod_assignmentques.util.slot.updateOneSlotSections();
        Y.Moodle.mod_assignmentques.util.slot.updateAllDependencyIcons();
    },

    NAME: 'mod_assignmentques-resource-toolbox',
    ATTRS: {
        courseid: {
            'value': 0
        },
...
M.mod_assignmentques.resource_toolbox = null;
M.mod_assignmentques.init_resource_toolbox = function(config) {
    M.mod_assignmentques.resource_toolbox = new RESOURCETOOLBOX(config);
    return M.mod_assignmentques.resource_toolbox;
};
/* global TOOLBOX, BODY, SELECTOR */
 ...
 *
 * @module moodle-mod_assignmentques-toolboxes
 * @namespace M.mod_assignmentques.toolboxes
 */
 ...
 *
 * This class is responsible for managing AJAX interactions with sections
 * when adding, editing, removing section headings when editing a assignmentques.
 *
 * @class section
 * @constructor
 * @extends M.mod_assignmentques.toolboxes.toolbox
 */
...
Y.extend(SECTIONTOOLBOX, TOOLBOX, {
     ...
     */
    initializer: function() {
       // M.mod_assignmentques.assignmentquesbase.register_module(this);
    ...
    delete_section_with_confirmation: function(ev, button, activity) {
        ...
        // Create the confirmation dialogue.
        var confirm = new M.core.confirm({
            question: M.util.get_string('confirmremovesectionheading', 'assignmentques', activity.get('aria-label')),
            modal: true
        });
            ...
            this.send_request(data, spinner, function(response) {
                if (response) {
                    ...
                    activity.one(SELECTOR.EDITSECTIONICON).set('title',
                            M.util.get_string('sectionheadingedit', 'assignmentques', response.instancesection));
                    activity.one(SELECTOR.EDITSECTIONICON).set('alt',
                            M.util.get_string('sectionheadingedit', 'assignmentques', response.instancesection));
                    var deleteicon = activity.one(SELECTOR.DELETESECTIONICON);
                    if (deleteicon) {
                        deleteicon.set('title', M.util.get_string('sectionheadingremove', 'assignmentques', response.instancesection));
                        deleteicon.set('alt', M.util.get_string('sectionheadingremove', 'assignmentques', response.instancesection));
                    }
                }
            });
        }
    ...
    NAME: 'mod_assignmentques-section-toolbox',
    ATTRS: {
        courseid: {
            'value': 0
        },
...
M.mod_assignmentques.init_section_toolbox = function(config) {
    ...
    "requires": [
        ...
        "event-key",
        "io",
        "moodle-mod_assignmentques-assignmentquesbase",
        "moodle-mod_assignmentques-util-slot",
