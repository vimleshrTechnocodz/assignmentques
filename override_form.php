...
/**
 * Settings form for overrides in the assignmentques module.
 *
...
require_once($CFG->dirroot . '/mod/assignmentques/mod_form.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class assignmentques_override_form extends moodleform {

    /** @var object course module object. */
    protected $cm;

    /** @var object the assignmentques settings object. */
     ...
     * @param object the assignmentques settings object.
     * @param context the assignmentques context.
     ...
     */
    public function __construct($submiturl, $cm, $assignmentques, $context, $groupmode, $override) {
    ...
    protected function definition() {
        ...
        $mform->addElement('header', 'override', get_string('override', 'assignmentques'));

        $assignmentquesgroupmode = groups_get_activity_groupmode($cm);
        $accessallgroups = ($assignmentquesgroupmode == NOGROUPS) || has_capability('moodle/site:accessallgroups', $this->context);
        ...
        if ($this->groupmode) {
            ...
            if ($this->groupid) {
                ...
                $mform->addElement('select', 'groupid',
                        get_string('overridegroup', 'assignmentques'), $groupchoices);
                ...
                if (empty($groups)) {
                    // Generate an error.
                    $link = new moodle_url('/mod/assignmentques/overrides.php', array('cmid'=>$cm->id));
                    print_error('groupsnone', 'assignmentques', $link);
                }
            ...
            if ($this->userid) {
                ...
                $mform->addElement('select', 'userid',
                        get_string('overrideuser', 'assignmentques'), $userchoices);
                $mform->freeze('userid');
            } else {
                ...
                if ($accessallgroups) {
                    $users = get_users_by_capability($this->context, 'mod/assignmentques:attempt',
                ...
                if (empty($users)) {
                    // Generate an error.
                    $link = new moodle_url('/mod/assignmentques/overrides.php', array('cmid'=>$cm->id));
                    print_error('usersnone', 'assignmentques', $link);
                }
        ...
        $mform->addElement('passwordunmask', 'password', get_string('requirepassword', 'assignmentques'));
        $mform->setType('password', PARAM_TEXT);
        $mform->addHelpButton('password', 'requirepassword', 'assignmentques');
        $mform->setDefault('password', $this->assignmentques->password);

        // Open and close dates.
        $mform->addElement('date_time_selector', 'timeopen',
                get_string('assignmentquesopen', 'assignmentques'), mod_assignmentques_mod_form::$datefieldoptions);
        $mform->setDefault('timeopen', $this->assignmentques->timeopen);

        $mform->addElement('date_time_selector', 'timeclose',
                get_string('assignmentquesclose', 'assignmentques'), mod_assignmentques_mod_form::$datefieldoptions);
        $mform->setDefault('timeclose', $this->assignmentques->timeclose);

        // Time limit.
        $mform->addElement('duration', 'timelimit',
                get_string('timelimit', 'assignmentques'), array('optional' => true));
        $mform->addHelpButton('timelimit', 'timelimit', 'assignmentques');
        $mform->setDefault('timelimit', $this->assignmentques->timelimit);
        ...
        $mform->addElement('select', 'attempts',
                get_string('attemptsallowed', 'assignmentques'), $attemptoptions);
        $mform->addHelpButton('attempts', 'attempts', 'assignmentques');
        $mform->setDefault('attempts', $this->assignmentques->attempts);

        // Submit buttons.
        $mform->addElement('submit', 'resetbutton',
                get_string('reverttodefaults', 'assignmentques'));

        $buttonarray = array();
        $buttonarray[] = $mform->createElement('submit', 'submitbutton',
                get_string('save', 'assignmentques'));
        $buttonarray[] = $mform->createElement('submit', 'againbutton',
                get_string('saveoverrideandstay', 'assignmentques'));
    ...
    public function validation($data, $files) {
        ...
        $mform =& $this->_form;
        $assignmentques = $this->assignmentques;
        ...
        // Ensure that at least one assignmentques setting was changed.
        $changed = false;
        $keys = array('timeopen', 'timeclose', 'timelimit', 'attempts', 'password');
        foreach ($keys as $key) {
            if ($data[$key] != $assignmentques->{$key}) {
                $changed = true;
                break;
            }
        }
        if (!$changed) {
            $errors['timeopen'] = get_string('nooverridedata', 'assignmentques');
        }
