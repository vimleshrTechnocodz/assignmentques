...
/**
 * Fallback page of /mod/assignmentques/edit.php add random question dialog,
...
list($thispageurl, $contexts, $cmid, $cm, $assignmentques, $pagevars) =
        question_edit_setup('editq', '/mod/assignmentques/addrandom.php', true);
...
if ($returnurl) {
    ...
    $returnurl = new moodle_url('/mod/assignmentques/edit.php', array('cmid' => $cmid));
}
if ($scrollpos) {
...
$mform = new assignmentques_add_random_form(new moodle_url('/mod/assignmentques/addrandom.php'),
...
if ($data = $mform->get_data()) {
    ...
    assignmentques_add_random_questions($assignmentques, $addonpage, $categoryid, $data->numbertoadd, $includesubcategories, $tagids);
...
// Setup $PAGE.
$streditingassignmentques = get_string('editinga', 'moodle', get_string('modulename', 'assignmentques'));
$PAGE->navbar->add($streditingassignmentques);
$PAGE->set_title($streditingassignmentques);
$PAGE->set_heading($course->fullname);
echo $OUTPUT->header();

if (!$assignmentquesname = $DB->get_field($cm->modname, 'name', array('id' => $cm->instance))) {
            print_error('invalidcoursemodule');
}

echo $OUTPUT->heading(get_string('addrandomquestiontoassignmentques', 'assignmentques', $assignmentquesname), 2);
$mform->display();
echo $OUTPUT->footer();
