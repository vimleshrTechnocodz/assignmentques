...
/**
 * Classes to enforce the various access rules that can apply to a assignmentques.
 *
 ...
 * assignmentques, with convinient methods for seeing whether access is allowed.
 *
 * @copyright 2009 Tim Hunt
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since     Moodle 2.2
 */
class assignmentques_access_manager {
    /** @var assignmentques the assignmentques settings object. */
    protected $assignmentquesobj;
    /** @var int the time to be considered as 'now'. */
    protected $timenow;
    /** @var array of assignmentques_access_rule_base. */
    protected $rules = array();

    /**
     * Create an instance for a particular assignmentques.
     * @param object $assignmentquesobj An instance of the class assignmentques from attemptlib.php.
     *      The assignmentques we will be controlling access to.
     * @param int $timenow The time to use as 'now'.
     * @param bool $canignoretimelimits Whether this user is exempt from time
     *      limits (has_capability('mod/assignmentques:ignoretimelimits', ...)).
     */
    public function __construct($assignmentquesobj, $timenow, $canignoretimelimits) {
        $this->assignmentquesobj = $assignmentquesobj;
        $this->timenow = $timenow;
        $this->rules = $this->make_rules($assignmentquesobj, $timenow, $canignoretimelimits);
    }

    /**
     * Make all the rules relevant to a particular assignmentques.
     * @param assignmentques $assignmentquesobj information about the assignmentques in question.
     * @param int $timenow the time that should be considered as 'now'.
     * @param bool $canignoretimelimits whether the current user is exempt from
     *      time limits by the mod/assignmentques:ignoretimelimits capability.
     * @return array of {@link assignmentques_access_rule_base}s.
     */
    protected function make_rules($assignmentquesobj, $timenow, $canignoretimelimits) {

        $rules = array();
        foreach (self::get_rule_classes() as $ruleclass) {
            $rule = $ruleclass::make($assignmentquesobj, $timenow, $canignoretimelimits);
            if ($rule) {
        ...
        foreach ($superceededrules as $superceededrule) {
            unset($rules['assignmentquesaccess_' . $superceededrule]);
        }
    ...
    protected static function get_rule_classes() {
        return core_component::get_plugin_list_with_class('assignmentquesaccess', '', 'rule.php');
    }
     ...
     * settings are stored in the assignmentques table.
     *
     * @param mod_assignmentques_mod_form $assignmentquesform the assignmentques settings form that is being built.
     * @param MoodleQuickForm $mform the wrapped MoodleQuickForm.
     */
    public static function add_settings_form_fields(
            mod_assignmentques_mod_form $assignmentquesform, MoodleQuickForm $mform) {

        foreach (self::get_rule_classes() as $rule) {
            $rule::add_settings_form_fields($assignmentquesform, $mform);
        }
    }
    ...
    public static function get_browser_security_choices() {
        $options = array('-' => get_string('none', 'assignmentques'));
        foreach (self::get_rule_classes() as $rule) {
     ...
     * @param mod_assignmentques_mod_form $assignmentquesform the assignmentques form object.
     * @return array $errors the updated $errors array.
     */
    public static function validate_settings_form_fields(array $errors,
            array $data, $files, mod_assignmentques_mod_form $assignmentquesform) {

        foreach (self::get_rule_classes() as $rule) {
            $errors = $rule::validate_settings_form_fields($errors, $data, $files, $assignmentquesform);
        }
    ...
    /**
     * Save any submitted settings when the assignmentques settings form is submitted.
     *
     * Note that the standard plugins do not use this mechanism because their
     * settings are stored in the assignmentques table.
     *
     * @param object $assignmentques the data from the assignmentques form, including $assignmentques->id
     *      which is the id of the assignmentques being saved.
     */
    public static function save_settings($assignmentques) {

        foreach (self::get_rule_classes() as $rule) {
            $rule::save_settings($assignmentques);
        }
    }

    /**
     * Delete any rule-specific settings when the assignmentques is deleted.
     *
     * Note that the standard plugins do not use this mechanism because their
     * settings are stored in the assignmentques table.
     *
     * @param object $assignmentques the data from the database, including $assignmentques->id
     *      which is the id of the assignmentques being deleted.
     * @since Moodle 2.7.1, 2.6.4, 2.5.7
     */
    public static function delete_settings($assignmentques) {

        foreach (self::get_rule_classes() as $rule) {
            $rule::delete_settings($assignmentques);
        }
    }
     ...
     */
    protected static function get_load_sql($assignmentquesid, $rules, $basefields) {
        $allfields = $basefields;
        $alljoins = '{assignmentques} assignmentques';
        $allparams = array('assignmentquesid' => $assignmentquesid);

        foreach ($rules as $rule) {
            list($fields, $joins, $params) = $rule::get_settings_sql($assignmentquesid);
            if ($fields) {
                if ($allfields) {
                    $allfields .= ', ';
                }
        ...
        return array("SELECT $allfields FROM $alljoins WHERE assignmentques.id = :assignmentquesid", $allparams);
    }
     ...
     */
    public static function load_settings($assignmentquesid) {
        global $DB;

        $rules = self::get_rule_classes();
        list($sql, $params) = self::get_load_sql($assignmentquesid, $rules, '');
        ...
        foreach ($rules as $rule) {
            $data += $rule::get_extra_settings($assignmentquesid);
        }
    ...
    /**
     * Load the assignmentques settings and any settings required by the access rules.
     ...
     *
     * @param int $assignmentquesid the assignmentques id.
     * @return object mdl_assignmentques row with extra fields.
     */
    public static function load_assignmentques_and_settings($assignmentquesid) {
        global $DB;

        $rules = self::get_rule_classes();
        list($sql, $params) = self::get_load_sql($assignmentquesid, $rules, 'assignmentques.*');
        $assignmentques = $DB->get_record_sql($sql, $params, MUST_EXIST);

        foreach ($rules as $rule) {
            foreach ($rule::get_extra_settings($assignmentquesid) as $name => $value) {
                $assignmentques->$name = $value;
            }
        }
    ...
    /**
     * Provide a description of the rules that apply to this assignmentques, such
     * as is shown at the top of the assignmentques view page. Note that not all
    ...
    /**
     * Whether or not a user should be allowed to start a new attempt at this assignmentques now.
     ...
     * @param int|null $attemptid the id of the current attempt, if there is one,
     *      otherwise null.
     * @return mod_assignmentques_preflight_check_form the form.
     */
    public function get_preflight_check_form(moodle_url $url, $attemptid) {
        ...
        $method = 'post';
        if (!empty($_GET['_qf__mod_assignmentques_preflight_check_form'])) {
            $method = 'get';
        }
        return new mod_assignmentques_preflight_check_form($url->out_omit_querystring(),
                array('rules' => $this->rules, 'assignmentquesobj' => $this->assignmentquesobj,
     ...
     * assignmentques. Used, for example, to change the label by the grade displayed on the view page from
     ...
     *
     * @param object $attempt the data from the relevant assignmentques_attempts row.
    ...
    /**
     * @return bolean if this assignmentques should only be shown to students in a popup window.
     */
    ...
    /**
     * Send the user back to the assignmentques view page. Normally this is just a redirect, but
     * If we were in a secure window, we close this window, and reload the view window we came from.
     *
     * This method does not return;
     *
     * @param mod_assignmentques_renderer $output the assignmentques renderer.
    ...
    public function back_to_view_page($output, $message = '') {
        if ($this->attempt_must_be_in_popup()) {
            echo $output->close_attempt_popup($this->assignmentquesobj->view_url(), $message);
            die();
        } else {
            redirect($this->assignmentquesobj->view_url(), $message);
        }
    }

    /**
     * Make some text into a link to review the assignmentques, if that is appropriate.
     *
    ...
    public function make_review_link($attempt, $reviewoptions, $output) {
        ...
        if (in_array($attempt->state, array(assignmentques_attempt::IN_PROGRESS, assignmentques_attempt::OVERDUE))) {
            return $output->no_review_message('');
        }

        $when = assignmentques_attempt_state($this->assignmentquesobj->get_assignmentques(), $attempt);
        $reviewoptions = mod_assignmentques_display_options::make_from_assignmentques(
                $this->assignmentquesobj->get_assignmentques(), $when);

        if (!$reviewoptions->attempt) {
            return $output->no_review_message($this->assignmentquesobj->cannot_review_message($when, true));

        } else {
            return $output->review_link($this->assignmentquesobj->review_url($attempt->id),
