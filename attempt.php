...
/**
 * This script displays a particular page of a assignmentques attempt that is in progress.
 *
...
if ($id = optional_param('id', 0, PARAM_INT)) {
    redirect($CFG->wwwroot . '/mod/assignmentques/startattempt.php?cmid=' . $id . '&sesskey=' . sesskey());
} else if ($qid = optional_param('q', 0, PARAM_INT)) {
    if (!$cm = get_coursemodule_from_instance('assignmentques', $qid)) {
        print_error('invalidassignmentquesid', 'assignmentques');
    }
    redirect(new moodle_url('/mod/assignmentques/startattempt.php',
...
if ($attemptobj->is_finished()) {
    redirect($attemptobj->review_url(null, $page));
} else if ($attemptobj->get_state() == assignmentques_attempt::OVERDUE) {
...
if (!$attemptobj->is_preview_user() && $messages) {
    print_error('attempterror', 'assignmentques', $attemptobj->view_url(),
...
$autosaveperiod = get_config('assignmentques', 'autosaveperiod');
if ($autosaveperiod) {
    $PAGE->requires->yui_module('moodle-mod_assignmentques-autosave',
            'M.mod_assignmentques.autosave.init', array($autosaveperiod));
}
...
$PAGE->requires->js_init_call('M.mod_assignmentques.init_attempt_form', null, false, assignmentques_get_js_module());

// Arrange for the navigation to be displayed in the first region on the page.
$navbc = $attemptobj->get_navigation_panel($output, 'assignmentques_attempt_nav_panel', $page);
