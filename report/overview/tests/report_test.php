...
/**
 * Tests for the assignmentques overview report.
 *
...
require_once($CFG->dirroot . '/mod/assignmentques/report/overview/report.php');
require_once($CFG->dirroot . '/mod/assignmentques/report/overview/tests/helpers.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class assignmentques_overview_report_testcase extends advanced_testcase {
    ...
    public function test_report_sql($isdownloading) {
        ...
        $assignmentques = $assignmentquesgenerator->create_instance(array('course' => $course->id,
                'grademethod' => ASSIGNMENTQUES_GRADEHIGHEST, 'grade' => 100.0, 'sumgrades' => 10.0,
                'attempts' => 10));
        ...
        assignmentques_add_assignmentques_question($q->id, $assignmentques, 0 , 10);
        ...
        // because they have a role with neither 'mod/assignmentques:attempt'
        // nor 'mod/assignmentques:reviewmyattempts'.
        ...
        $attempts = array(
            array($assignmentques, $student1, 1, 0.0,  assignmentques_attempt::FINISHED),
            array($assignmentques, $student1, 2, 5.0,  assignmentques_attempt::FINISHED),
            array($assignmentques, $student1, 3, 8.0,  assignmentques_attempt::FINISHED),
            array($assignmentques, $student1, 4, null, assignmentques_attempt::ABANDONED),
            array($assignmentques, $student1, 5, null, assignmentques_attempt::IN_PROGRESS),
            array($assignmentques, $student2, 1, null, assignmentques_attempt::ABANDONED),
            array($assignmentques, $student2, 2, null, assignmentques_attempt::ABANDONED),
            array($assignmentques, $student2, 3, 7.0,  assignmentques_attempt::FINISHED),
            array($assignmentques, $student2, 4, null, assignmentques_attempt::ABANDONED),
            array($assignmentques, $student2, 5, null, assignmentques_attempt::ABANDONED),
        );

        // Load it in to assignmentques attempts table.
        foreach ($attempts as $attemptdata) {
            list($assignmentques, $student, $attemptnumber, $sumgrades, $state) = $attemptdata;
            ...
            $attempt = assignmentques_create_attempt($assignmentquesobj, $attemptnumber, null, $timestart, false, $student->id);

            $attempt = assignmentques_start_new_attempt($assignmentquesobj, $quba, $attempt, $attemptnumber, $timestamp);
            ...
            switch ($state) {
                ...
                case assignmentques_attempt::FINISHED:
                    ...
                    $DB->update_record('assignmentques_attempts', $update);
                    assignmentques_save_best_grade($attemptobj->get_assignmentques(), $student->id);
                    break;
            }
        }
        ...
        $qmsubselect = assignmentques_report_qm_filter_select($assignmentques);
        $studentsjoins = get_enrolled_with_capabilities_join($context, '',
                array('mod/assignmentques:attempt', 'mod/assignmentques:reviewmyattempts'));
        $empty = new \core\dml\sql_join();

        // Set the options.
        $reportoptions = new assignmentques_overview_options('overview', $assignmentques, $cm, null);
        $reportoptions->attempts = assignmentques_attempts_report::ENROLLED_ALL;
        $reportoptions->onlygraded = true;
        $reportoptions->states = array(assignmentques_attempt::IN_PROGRESS, assignmentques_attempt::OVERDUE, assignmentques_attempt::FINISHED);

        // Now do a minimal set-up of the table class.
        $q->slot = 1;
        $q->maxmark = 10;
        $table = new assignmentques_overview_table($assignmentques, $context, $qmsubselect, $reportoptions,
        ...
        $table->define_baseurl(new moodle_url('/mod/assignmentques/report.php'));
        $table->setup();
    ...
    public function test_get_bands_count_and_width($grade, $expected) {
        ...
        $assignmentques = $assignmentquesgenerator->create_instance(['course' => SITEID, 'grade' => $grade]);
        $this->assertEquals($expected, assignmentques_overview_report::get_bands_count_and_width($assignmentques));
    }
    ...
    public function test_delete_selected_attempts() {
        ...
        $context = context_module::instance($assignmentques->cmid);
        $cm = get_coursemodule_from_id('assignmentques', $assignmentques->cmid);
        $allowedjoins = get_enrolled_with_capabilities_join($context, '', ['mod/assignmentques:attempt', 'mod/assignmentques:reviewmyattempts']);
        $assignmentquesattemptsreport = new testable_assignmentques_attempts_report();

        // Create the new attempt and initialize the question sessions.
        $assignmentquesobj = assignmentques::create($assignmentques->id, $student->id);
        $quba = question_engine::make_questions_usage_by_activity('mod_assignmentques', $assignmentquesobj->get_context());
        $quba->set_preferred_behaviour($assignmentquesobj->get_assignmentques()->preferredbehaviour);
        $attempt = assignmentques_create_attempt($assignmentquesobj, 1, null, $timestart, false, $student->id);
        $attempt = assignmentques_start_new_attempt($assignmentquesobj, $quba, $attempt, 1, $timestamp);
        $attempt = assignmentques_attempt_save_started($assignmentquesobj, $quba, $attempt);

        // Delete the student's attempt.
        $assignmentquesattemptsreport->delete_selected_attempts($assignmentques, $cm, [$attempt->id], $allowedjoins);
    }
