 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class assignmentques_report_responses_from_steps_testcase extends mod_assignmentques_attempt_walkthrough_from_csv_testcase {
     ...
     *
     * @param array $assignmentquessettings settings to override default settings for assignmentques created by generator. Taken from assignmentqueszes.csv.
    ...
    public function test_walkthrough_from_csv($assignmentquessettings, $csvdata) {
        ...
        $assignmentquesattemptids = $this->walkthrough_attempts($csvdata['steps']);
        ...
        for ($rowno = 0; $rowno < $csvdata['responses']->getRowCount(); $rowno++) {
            ...
            if (!isset($assignmentquesattemptids[$responses['assignmentquesattempt']])) {
                throw new coding_exception("There is no assignmentquesattempt {$responses['assignmentquesattempt']}!");
            }
            $this->assert_response_test($assignmentquesattemptids[$responses['assignmentquesattempt']], $responses);
        }
    }

    protected function assert_response_test($assignmentquesattemptid, $responses) {
        $assignmentquesattempt = assignmentques_attempt::create($assignmentquesattemptid);
        ...
        foreach ($responses['slot'] as $slot => $tests) {
            ...
            $qa = $assignmentquesattempt->get_question_attempt($slot);
            ...
            if (null === $step) {
                throw new coding_exception("There is no step no {$responses['submittedstepno']} ".
                                           "for slot $slot in assignmentquesattempt {$responses['assignmentquesattempt']}!");
            }
            foreach (array('responsesummary', 'fraction', 'state') as $column) {
                if (isset($tests[$column]) && $tests[$column] != '') {
                    ...
                    $failuremessage = "Error in  assignmentquesattempt {$responses['assignmentquesattempt']} in $column, slot $slot, ".
