...
/**
 * Base class for the table used by a {@link assignmentques_attempts_report}.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class assignmentques_attempts_report_table extends table_sql {
    ...
    /** @var object the assignmentques settings for the assignmentques we are reporting on. */
    ...
    /** @var object mod_assignmentques_attempts_report_options the options affecting this report. */
    protected $options;
    ...
    /** @var object the questions that comprise this assignmentques.. */
    protected $questions;
     ...
     * @param mod_assignmentques_attempts_report_options $options
     ...
     */
    public function __construct($uniqueid, $assignmentques, $context, $qmsubselect,
            mod_assignmentques_attempts_report_options $options, \core\dml\sql_join $groupstudentsjoins, \core\dml\sql_join $studentsjoins,
            $questions, $reporturl) {
    ...
    public function col_fullname($attempt) {
        ...
        return $html . html_writer::empty_tag('br') . html_writer::link(
                new moodle_url('/mod/assignmentques/review.php', array('attempt' => $attempt->attempt)),
                get_string('reviewattempt', 'assignmentques'), array('class' => 'reviewlink'));
    }
    ...
    public function col_state($attempt) {
        if (!is_null($attempt->attempt)) {
            return assignmentques_attempt::state_name($attempt->state);
        } else {
            return  '-';
        }
    }
    ...
    public function col_feedbacktext($attempt) {
        ...
        $feedback = assignmentques_report_feedback_for_grade(
                assignmentques_rescale_grade($attempt->sumgrades, $this->assignmentques, false),
                $this->assignmentques->id, $this->context);
    ...
    public function make_review_link($data, $attempt, $slot) {
        ...
        $url = new moodle_url('/mod/assignmentques/reviewquestion.php', $reviewparams);
        $output = $OUTPUT->action_link($url, $output,
                new popup_action('click', $url, 'reviewquestion',
                        array('height' => 450, 'width' => 650)),
                array('title' => get_string('reviewresponse', 'assignmentques')));
    ...
    public function base_sql(\core\dml\sql_join $allowedstudentsjoins) {
        ...
        // Please note this uniqueid column is not the same as assignmentquesa.uniqueid.
        $fields = 'DISTINCT ' . $DB->sql_concat('u.id', "'#'", 'COALESCE(assignmentquesa.attempt, 0)') . ' AS uniqueid,';
        ...
        $fields .= '
                assignmentquesa.uniqueid AS usageid,
                assignmentquesa.id AS attempt,
                u.id AS userid,
                u.idnumber, ' . $allnames . ',
                u.picture,
                u.imagealt,
                ...
                u.department,
                u.email' . $extrafields . ',
                assignmentquesa.state,
                assignmentquesa.sumgrades,
                assignmentquesa.timefinish,
                assignmentquesa.timestart,
                CASE WHEN assignmentquesa.timefinish = 0 THEN null
                     WHEN assignmentquesa.timefinish > assignmentquesa.timestart THEN assignmentquesa.timefinish - assignmentquesa.timestart
                     ELSE 0 END AS duration';
            // To explain that last bit, timefinish can be non-zero and less
            // than timestart when you have two load-balanced servers with very
            // badly synchronised clocks, and a student does a really quick attempt.

        // This part is the same for all cases. Join the users and assignmentques_attempts tables.
        $from = " {user} u";
        $from .= "\nLEFT JOIN {assignmentques_attempts} assignmentquesa ON
                                    assignmentquesa.userid = u.id AND assignmentquesa.assignmentques = :assignmentquesid";
        $params = array('assignmentquesid' => $this->assignmentques->id);

        if ($this->qmsubselect && $this->options->onlygraded) {
            $from .= " AND (assignmentquesa.state <> :finishedstate OR $this->qmsubselect)";
            $params['finishedstate'] = assignmentques_attempt::FINISHED;
        }

        switch ($this->options->attempts) {
            case assignmentques_attempts_report::ALL_WITH:
                // Show all attempts, including students who are no longer in the course.
                $where = 'assignmentquesa.id IS NOT NULL AND assignmentquesa.preview = 0';
                break;
            case assignmentques_attempts_report::ENROLLED_WITH:
                // Show only students with attempts.
                $from .= "\n" . $allowedstudentsjoins->joins;
                $where = "assignmentquesa.preview = 0 AND assignmentquesa.id IS NOT NULL AND " . $allowedstudentsjoins->wheres;
                $params = array_merge($params, $allowedstudentsjoins->params);
                break;
            case assignmentques_attempts_report::ENROLLED_WITHOUT:
                // Show only students without attempts.
                $from .= "\n" . $allowedstudentsjoins->joins;
                $where = "assignmentquesa.id IS NULL AND " . $allowedstudentsjoins->wheres;
                $params = array_merge($params, $allowedstudentsjoins->params);
                break;
            case assignmentques_attempts_report::ENROLLED_ALL:
                // Show all students with or without attempts.
                $from .= "\n" . $allowedstudentsjoins->joins;
                $where = "(assignmentquesa.preview = 0 OR assignmentquesa.preview IS NULL) AND " . $allowedstudentsjoins->wheres;
                $params = array_merge($params, $allowedstudentsjoins->params);
                break;
        }
        ...
        if ($this->options->states) {
            ...
            $params += $stateparams;
            $where .= " AND (assignmentquesa.state $statesql OR assignmentquesa.state IS NULL)";
        }
    ...
    protected function add_latest_state_join($slot) {
        ...
        $qubaids = new qubaid_join("{assignmentques_attempts} {$alias}assignmentquesa", "{$alias}assignmentquesa.uniqueid",
                "{$alias}assignmentquesa.assignmentques = :{$alias}assignmentquesid", array("{$alias}assignmentquesid" => $this->sql->params['assignmentquesid']));
        ...
        $this->sql->from .= "\nLEFT JOIN $inlineview ON " .
                "$alias.questionusageid = assignmentquesa.uniqueid AND $alias.slot = :{$alias}slot";
    ...
    protected function get_qubaids_condition() {
        ...
        if ($this->is_downloading()) {
            ...
            return new qubaid_join("(
                SELECT DISTINCT assignmentquesa.uniqueid
                  FROM " . $this->sql->from . "
                 WHERE " . $this->sql->where . "
                    ) assignmentquesasubquery", 'assignmentquesasubquery.uniqueid',
                    "1 = 1", $this->sql->params);
        }
    ...
    public function get_sort_columns() {
        ...
        $sortcolumns['assignmentquesa.id'] = SORT_ASC;
        return $sortcolumns;
    }
    ...
    public function wrap_html_finish() {
        ...
        echo '<a id="checkattempts" href="#">' .
                get_string('selectall', 'assignmentques') . '</a> / ';
        echo '<a id="uncheckattempts" href="#">' .
                get_string('selectnone', 'assignmentques') . '</a> ';
