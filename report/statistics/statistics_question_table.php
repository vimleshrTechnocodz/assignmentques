...
/**
 * Assignmentques statistics report, table for showing response analysis for a particular question (or sub question).
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class assignmentques_statistics_question_table extends flexible_table {
    ...
    public function __construct($qid) {
        parent::__construct('mod-assignmentques-report-statistics-question-table' . $qid);
    }
    ...
    public function question_setup($reporturl, $questiondata, $s, $responseanalysis) {
        ...
        $this->set_attribute('class', 'generaltable generalbox boxaligncenter assignmentquesresponseanalysis');
        ...
        if ($responseanalysis->has_subparts()) {
            $columns[] = 'part';
            $headers[] = get_string('partofquestion', 'assignmentques_statistics');
        }

        if ($responseanalysis->has_multiple_response_classes()) {
            $columns[] = 'responseclass';
            $headers[] = get_string('modelresponse', 'assignmentques_statistics');

            if ($responseanalysis->has_actual_responses()) {
                $columns[] = 'response';
                $headers[] = get_string('actualresponse', 'assignmentques_statistics');
            }

        } else {
            $columns[] = 'response';
            $headers[] = get_string('response', 'assignmentques_statistics');
        }

        $columns[] = 'fraction';
        $headers[] = get_string('optiongrade', 'assignmentques_statistics');

        if (!$responseanalysis->has_multiple_tries_data()) {
            $columns[] = 'totalcount';
            $headers[] = get_string('count', 'assignmentques_statistics');
        } else {
            $countcolumns = range(1, $responseanalysis->get_maximum_tries());
            foreach ($countcolumns as $countcolumn) {
                $columns[] = 'trycount'.$countcolumn;
                $headers[] = get_string('counttryno', 'assignmentques_statistics', $countcolumn);
            }
        }

        $columns[] = 'frequency';
        $headers[] = get_string('frequency', 'assignmentques_statistics');
