...
/**
 * Assignmentques statistics report, table for showing statistics of each question in the assignmentques.
 *
...
/**
 * This table has one row for each question in the assignmentques, with sub-rows when
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class assignmentques_statistics_table extends flexible_table {
    /** @var object the assignmentques settings. */
    protected $assignmentques;

    /** @var integer the assignmentques course_module id. */
    protected $cmid;
    ...
    public function __construct() {
        parent::__construct('mod-assignmentques-report-statistics-report');
    }
     ...
     * @param int $cmid the assignmentques course_module id
     * @param moodle_url $reporturl the URL to redisplay this report.
     * @param int $s number of attempts included in the statistics.
     */
    public function statistics_setup($assignmentques, $cmid, $reporturl, $s) {
        ...
        $columns[] = 'number';
        $headers[] = get_string('questionnumber', 'assignmentques_statistics');
        ...
        if (!$this->is_downloading()) {
            ...
            $columns[] = 'qtype';
            $headers[] = get_string('questiontype', 'assignmentques_statistics');
        }

        $columns[] = 'name';
        $headers[] = get_string('questionname', 'assignmentques');

        $columns[] = 's';
        $headers[] = get_string('attempts', 'assignmentques_statistics');

        if ($s > 1) {
            $columns[] = 'facility';
            $headers[] = get_string('facility', 'assignmentques_statistics');

            $columns[] = 'sd';
            $headers[] = get_string('standarddeviationq', 'assignmentques_statistics');
        }

        $columns[] = 'random_guess_score';
        $headers[] = get_string('random_guess_score', 'assignmentques_statistics');

        $columns[] = 'intended_weight';
        $headers[] = get_string('intended_weight', 'assignmentques_statistics');

        $columns[] = 'effective_weight';
        $headers[] = get_string('effective_weight', 'assignmentques_statistics');

        $columns[] = 'discrimination_index';
        $headers[] = get_string('discrimination_index', 'assignmentques_statistics');

        $columns[] = 'discriminative_efficiency';
        $headers[] = get_string('discriminative_efficiency', 'assignmentques_statistics');
    ...
    protected function col_actions($questionstat) {
        if ($this->is_calculated_question_summary($questionstat)) {
            ...
            return assignmentques_question_action_icons($this->assignmentques, $this->cmid,
    ...
    protected function col_name($questionstat) {
        ...
        if (!is_null($questionstat->variant)) {
            ...
            $name = get_string('nameforvariant', 'assignmentques_statistics', $a);
        }
        ...
        if (!is_null($questionstat->variant)) {
            ...
            if ($questionstat->subquestion && !$questionstat->get_variants()) {
                ...
                $name = html_writer::link($url, $name, array('title' => get_string('detailedanalysis', 'assignmentques_statistics')));
                ...
                if ($this->is_calculated_question_summary($questionstat)) {
                    ...
                    // in the UI.
                    $name = html_writer::div(get_string('rangeofvalues', 'assignmentques_statistics'));
                    ...
                    $name = html_writer::link($url,
                                              $name,
                                              array('title' => get_string('detailedanalysis', 'assignmentques_statistics')));
                }
            }
        }
        ...
        if ($this->is_calculated_question_summary($questionstat)) {
            $name .= html_writer::link($url, get_string('viewanalysis', 'assignmentques_statistics'));
        } else if (!empty($questionstat->minmedianmaxnotice)) {
            $name = get_string($questionstat->minmedianmaxnotice, 'assignmentques_statistics') . '<br />' . $name;
        }
     ...
     * of maximum mark for the assignmentques. That is, the indended influence this question
    ...
    protected function col_intended_weight($questionstat) {
        if ($this->is_calculated_question_summary($questionstat)) {
            ...
            if (is_null($min) && is_null($max)) {
                ...
                $min = assignmentques_report_scale_summarks_as_percentage($min, $this->assignmentques);
                $max = assignmentques_report_scale_summarks_as_percentage($max, $this->assignmentques);
                return $this->format_range($min, $max);
            }
        } else {
            return assignmentques_report_scale_summarks_as_percentage($questionstat->maxmark, $this->assignmentques);
        }
    }
    ...
    protected function col_effective_weight($questionstat) {
        ...
        if ($this->is_calculated_question_summary($questionstat)) {
            ...
            if (is_null($min) && is_null($max)) {
                ...
                if ($negcovar) {
                    $min = get_string('negcovar', 'assignmentques_statistics');
                }
            ...
            $negcovar = get_string('negcovar', 'assignmentques_statistics');

            if (!$this->is_downloading()) {
                $negcovar = html_writer::tag('div',
                        $negcovar . $OUTPUT->help_icon('negcovar', 'assignmentques_statistics'),
     ...
     * other questions in this assignmentques.
     * @param \core_question\statistics\questions\calculated $questionstat stats for the question.
     * @return string contents of this table cell.
     */
    ...
    protected function format_range(string $min = null, string $max = null) {
        if (is_null($min) && is_null($max)) {
            ...
            return get_string('rangebetween', 'assignmentques_statistics', $a);
        }
    }
