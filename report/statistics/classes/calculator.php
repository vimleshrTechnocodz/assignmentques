...
namespace assignmentques_statistics;
defined('MOODLE_INTERNAL') || die();

/**
 * Class to calculate and also manage caching of assignmentques statistics.
 *
 * These assignmentques statistics calculations are described here :
 *
 * http://docs.moodle.org/dev/Assignmentques_statistics_calculations#Test_statistics
 *
...
class calculator {
    ...
    /**
     * Compute the assignmentques statistics.
     *
     ...
     * @return calculated $assignmentquesstats The statistics for overall attempt scores.
     */
    public function calculate($assignmentquesid, $whichattempts, \core\dml\sql_join $groupstudentsjoins, $p, $sumofmarkvariance) {

        $this->progress->start_progress('', 3);

        $assignmentquesstats = new calculated($whichattempts);

        $countsandaverages = $this->attempt_counts_and_averages($assignmentquesid, $groupstudentsjoins);
        $this->progress->progress(1);

        foreach ($countsandaverages as $propertyname => $value) {
            $assignmentquesstats->{$propertyname} = $value;
        }

        $s = $assignmentquesstats->s();
        if ($s != 0) {

            // Recalculate sql again this time possibly including test for first attempt.
            list($fromqa, $whereqa, $qaparams) =
                assignmentques_statistics_attempts_sql($assignmentquesid, $groupstudentsjoins, $whichattempts);

            $assignmentquesstats->median = $this->median($s, $fromqa, $whereqa, $qaparams);
            $this->progress->progress(2);

            if ($s > 1) {

                $powers = $this->sum_of_powers_of_difference_to_mean($assignmentquesstats->avg(), $fromqa, $whereqa, $qaparams);
                $this->progress->progress(3);

                $assignmentquesstats->standarddeviation = sqrt($powers->power2 / ($s - 1));

                // Skewness.
                if ($s > 2) {
                    // See http://docs.moodle.org/dev/Assignmentques_item_analysis_calculations_in_practise#Skewness_and_Kurtosis.
                    $m2 = $powers->power2 / $s;
                    $m3 = $powers->power3 / $s;
                    $m4 = $powers->power4 / $s;
                    ...
                    $k3 = $s * $s * $m3 / (($s - 1) * ($s - 2));
                    if ($k2 != 0) {
                        $assignmentquesstats->skewness = $k3 / (pow($k2, 3 / 2));

                        // Kurtosis.
                        if ($s > 3) {
                            $k4 = $s * $s * ((($s + 1) * $m4) - (3 * ($s - 1) * $m2 * $m2)) / (($s - 1) * ($s - 2) * ($s - 3));
                            $assignmentquesstats->kurtosis = $k4 / ($k2 * $k2);
                        }

                        if ($p > 1) {
                            $assignmentquesstats->cic = (100 * $p / ($p - 1)) * (1 - ($sumofmarkvariance / $k2));
                            $assignmentquesstats->errorratio = 100 * sqrt(1 - ($assignmentquesstats->cic / 100));
                            $assignmentquesstats->standarderror = $assignmentquesstats->errorratio *
                                $assignmentquesstats->standarddeviation / 100;
                        }
                    }
            ...
            $assignmentquesstats->cache(assignmentques_statistics_qubaids_condition($assignmentquesid, $groupstudentsjoins, $whichattempts));
        }
        $this->progress->end_progress();
        return $assignmentquesstats;
    }
    ...
    public function get_cached($qubaids) {
        ...
        $fromdb = $DB->get_record_select('assignmentques_statistics', 'hashcode = ? AND timemodified > ?',
    ...
    public function get_last_calculated_time($qubaids) {
        ...
        $timemodified = time() - self::TIME_TO_CACHE;
        return $DB->get_field_select('assignmentques_statistics', 'timemodified', 'hashcode = ? AND timemodified > ?',
                                         array($qubaids->get_hash_code(), $timemodified));
    }

    /**
     * Given a particular assignmentques grading method return a lang string describing which attempts contribute to grade.
     *
    ...
    public static function using_attempts_lang_string($whichattempts) {
         return get_string(static::using_attempts_string_id($whichattempts), 'assignmentques_statistics');
    }

    /**
     * Given a particular assignmentques grading method return a string id for use as a field name prefix in mdl_assignmentques_statistics or to
     ...
     *
     * See : http://docs.moodle.org/dev/Assignmentques_item_analysis_calculations_in_practise
     ...
     */
    protected function attempt_counts_and_averages($assignmentquesid, \core\dml\sql_join $groupstudentsjoins) {
        global $DB;

        $attempttotals = new \stdClass();
        foreach (array_keys(assignmentques_get_grading_options()) as $which) {

            list($fromqa, $whereqa, $qaparams) = assignmentques_statistics_attempts_sql($assignmentquesid, $groupstudentsjoins, $which);
     ...
     * Median mark.
     *
     * http://docs.moodle.org/dev/Assignmentques_statistics_calculations#Median_Score
     *
    ...
    protected function median($s, $fromqa, $whereqa, $qaparams) {
        ...
        $sql = "SELECT assignmentquesa.id, assignmentquesa.sumgrades
                  FROM $fromqa
                 WHERE $whereqa
              ORDER BY sumgrades";
     ...
     *
     * Explanation here : http://docs.moodle.org/dev/Assignmentques_item_analysis_calculations_in_practise
    ...
    protected function sum_of_powers_of_difference_to_mean($mean, $fromqa, $whereqa, $qaparams) {
        ...
        $sql = "SELECT
                    SUM(POWER((assignmentquesa.sumgrades - $mean), 2)) AS power2,
                    SUM(POWER((assignmentquesa.sumgrades - $mean), 3)) AS power3,
                    SUM(POWER((assignmentquesa.sumgrades - $mean), 4)) AS power4
                  FROM $fromqa
                 WHERE $whereqa";
