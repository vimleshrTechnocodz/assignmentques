 ...
 *
 * The assignmentques stats below and the question stats found in qstats00.csv were calculated independently in a spreadsheet which is
 * available in open document or excel format here :
 * https://github.com/jamiepratt/moodle-assignmentques-tools/tree/master/statsspreadsheet
 *
...
global $CFG;
require_once($CFG->dirroot . '/mod/assignmentques/tests/attempt_walkthrough_from_csv_test.php');
require_once($CFG->dirroot . '/mod/assignmentques/report/default.php');
require_once($CFG->dirroot . '/mod/assignmentques/report/statistics/report.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class assignmentques_report_statistics_from_steps_testcase extends mod_assignmentques_attempt_walkthrough_from_csv_testcase {

    /**
     * @var assignmentques_statistics_report object to do stats calculations.
     */
    protected $report;
    ...
    /**
     * Create a assignmentques add questions to it, walk through assignmentques attempts and then check results.
     *
    ...
    public function test_walkthrough_from_csv($assignmentquessettings, $csvdata) {
        ...
        $whichattempts = ASSIGNMENTQUES_GRADEAVERAGE; // All attempts.
        $whichtries = question_attempt::ALL_TRIES;
        $groupstudentsjoins = new \core\dml\sql_join();
        list($questions, $assignmentquesstats, $questionstats, $qubaids) =
                    $this->check_stats_calculations_and_response_analysis($csvdata,
                            $whichattempts, $whichtries, $groupstudentsjoins);
        if ($assignmentquessettings['testnumber'] === '00') {
            $this->check_variants_count_for_assignmentques_00($questions, $questionstats, $whichtries, $qubaids);
            $this->check_assignmentques_stats_for_assignmentques_00($assignmentquesstats);
        }
    }
     ...
     * @param $qubaids
     */
    protected function check_variants_count_for_assignmentques_00($questions, $questionstats, $whichtries, $qubaids) {
    ...
    /**
     * @param $assignmentquesstats
     */
    protected function check_assignmentques_stats_for_assignmentques_00($assignmentquesstats) {
        $assignmentquesstatsexpected = array(
            'median'             => 4.5,
        ...
        foreach ($assignmentquesstatsexpected as $statname => $statvalue) {
            $this->assertEquals($statvalue, $assignmentquesstats->$statname, $assignmentquesstats->$statname, abs($statvalue) * 1.5e-5);
        }
    }
     ...
     * @return array with contents 0 => $questions, 1 => $assignmentquesstats, 2=> $questionstats, 3=> $qubaids Might be needed for further
     *               testing.
     */
    protected function check_stats_calculations_and_response_analysis($csvdata, $whichattempts, $whichtries,
        ...
        $this->report = new assignmentques_statistics_report();
        $questions = $this->report->load_and_initialise_questions_for_calculations($this->assignmentques);
        list($assignmentquesstats, $questionstats) = $this->report->get_all_stats_and_analysis($this->assignmentques,
                                                                                     $whichattempts,
                                                                                     $whichtries,
                                                                                     $groupstudentsjoins,
                                                                                     $questions);

        $qubaids = assignmentques_statistics_qubaids_condition($this->assignmentques->id, $groupstudentsjoins, $whichattempts);

        // We will create some assignmentques and question stat calculator instances and some response analyser instances, just in order
        // to check the last analysed time then returned.
        $assignmentquescalc = new \assignmentques_statistics\calculator();
        // Should not be a delay of more than one second between the calculation of stats above and here.
        $this->assertTimeCurrent($assignmentquescalc->get_last_calculated_time($qubaids));
        ...
        if (isset($csvdata['qstats'])) {
            ...
            return array($questions, $assignmentquesstats, $questionstats, $qubaids);
        }
        return array($questions, $assignmentquesstats, $questionstats, $qubaids);
    }
