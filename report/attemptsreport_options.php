...
/**
 * Base class for the options that control what is visible in an {@link assignmentques_attempts_report}.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_assignmentques_attempts_report_options {

    /** @var string the report mode. */
    public $mode;

    /** @var object the settings for the assignmentques being reported on. */
    public $assignmentques;

    /** @var object the course module objects for the assignmentques being reported on. */
    public $cm;

    /** @var object the course settings for the course the assignmentques is in. */
    public $course;

    /**
     * @var array form field name => corresponding assignmentques_attempt:: state constant.
     */
    protected static $statefields = array(
        'stateinprogress' => assignmentques_attempt::IN_PROGRESS,
        'stateoverdue'    => assignmentques_attempt::OVERDUE,
        'statefinished'   => assignmentques_attempt::FINISHED,
        'stateabandoned'  => assignmentques_attempt::ABANDONED,
    );

    /**
     * @var string assignmentques_attempts_report::ALL_WITH or assignmentques_attempts_report::ENROLLED_WITH
     *      assignmentques_attempts_report::ENROLLED_WITHOUT or assignmentques_attempts_report::ENROLLED_ALL
     */
    public $attempts = assignmentques_attempts_report::ENROLLED_WITH;

    /** @var int the currently selected group. 0 if no group is selected. */
    public $group = 0;

    /**
     * @var array|null of assignmentques_attempt::IN_PROGRESS, etc. constants. null means
     *      no restriction.
     */
    public $states = array(assignmentques_attempt::IN_PROGRESS, assignmentques_attempt::OVERDUE,
            assignmentques_attempt::FINISHED, assignmentques_attempt::ABANDONED);
    ...
    public $pagesize = assignmentques_attempts_report::DEFAULT_PAGE_SIZE;
     ...
     * @param object $assignmentques the settings for the assignmentques being reported on.
     * @param object $cm the course module objects for the assignmentques being reported on.
     * @param object $coures the course settings for the coures this assignmentques is in.
     */
    public function __construct($mode, $assignmentques, $cm, $course) {
        $this->mode   = $mode;
        $this->assignmentques   = $assignmentques;
        $this->cm     = $cm;
        $this->course = $course;

        $this->usercanseegrades = assignmentques_report_should_show_grades($assignmentques, context_module::instance($cm->id));
    }
    ...
    public function get_url() {
        return new moodle_url('/mod/assignmentques/report.php', $this->get_url_params());
    }
    ...
    public function setup_from_user_preferences() {
        $this->pagesize = get_user_preferences('assignmentques_report_pagesize', $this->pagesize);
    }
    ...
    public function update_user_preferences() {
        set_user_preference('assignmentques_report_pagesize', $this->pagesize);
    }
    ...
    public function resolve_dependencies() {
        if ($this->group) {
            ...
            if ($this->attempts === null || $this->attempts == assignmentques_attempts_report::ALL_WITH) {
                $this->attempts = assignmentques_attempts_report::ENROLLED_WITH;
            }

        } else if (!$this->group && $this->course->id == SITEID) {
            // Force report on front page to show all, unless a group is selected.
            $this->attempts = assignmentques_attempts_report::ALL_WITH;

        } else if (!in_array($this->attempts, array(assignmentques_attempts_report::ALL_WITH, assignmentques_attempts_report::ENROLLED_WITH,
                assignmentques_attempts_report::ENROLLED_WITHOUT, assignmentques_attempts_report::ENROLLED_ALL))) {
            $this->attempts = assignmentques_attempts_report::ENROLLED_WITH;
        }
        ...
        if (!assignmentques_report_can_filter_only_graded($this->assignmentques)) {
        ...
        if ($this->attempts == assignmentques_attempts_report::ENROLLED_WITHOUT) {
            $this->states = null;
            $this->onlygraded = false;
        }
        ...
        if ($this->pagesize < 1) {
            $this->pagesize = assignmentques_attempts_report::DEFAULT_PAGE_SIZE;
        }
    }
    ...
    protected function is_showing_finished_attempts() {
        return $this->states === null || in_array(assignmentques_attempt::FINISHED, $this->states);
    }
}
