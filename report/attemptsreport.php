...
/**
 * Base class for assignmentques reports that are basically a table with one row for each attempt.
 *
 * @copyright 2010 The Open University
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class assignmentques_attempts_report extends assignmentques_default_report {
    ...
    /** @var object the assignmentques context. */
    protected $context;

    /** @var mod_assignmentques_attempts_report_form The settings form to use. */
    protected $form;
     ...
     */
    protected function init($mode, $formclass, $assignmentques, $cm, $course) {
        $this->mode = $mode;
        ...
        $this->qmsubselect = assignmentques_report_qm_filter_select($assignmentques);

        $this->form = new $formclass($this->get_base_url(),
                array('assignmentques' => $assignmentques, 'currentgroup' => $currentgroup, 'context' => $this->context));
    ...
    protected function get_students_joins($cm, $course = null) {
        ...
        $groupstudentsjoins = get_enrolled_with_capabilities_join($this->context, '',
                array('mod/assignmentques:attempt', 'mod/assignmentques:reviewmyattempts'), $currentgroup);

        return array($currentgroup, $studentsjoins, $groupstudentsjoins, $groupstudentsjoins);
    }

    /**
     * Outputs the things you commonly want at the top of a assignmentques report.
     *
     ...
     * @param mod_assignmentques_attempts_report_options $options the current report settings.
     * @param int $currentgroup the current group.
     * @param bool $hasquestions whether there are any questions in the assignmentques.
     * @param bool $hasstudents whether there are any relevant students.
     */
    protected function print_standard_header_and_messages($cm, $course, $assignmentques,
            $options, $currentgroup, $hasquestions, $hasstudents) {
        global $OUTPUT;

        $this->print_header_and_tabs($cm, $course, $assignmentques, $this->mode);
        ...
        if ($strattemptnum = assignmentques_num_attempt_summary($assignmentques, $cm, true, $currentgroup)) {
            echo '<div class="assignmentquesattemptcounts">' . $strattemptnum . '</div>';
        }
    ...
    protected function add_state_column(&$columns, &$headers) {
        $columns[] = 'state';
        $headers[] = get_string('attemptstate', 'assignmentques');
    }
    ...
    protected function add_time_columns(&$columns, &$headers) {
        $columns[] = 'timestart';
        $headers[] = get_string('startedon', 'assignmentques');

        $columns[] = 'timefinish';
        $headers[] = get_string('timecompleted', 'assignmentques');

        $columns[] = 'duration';
        $headers[] = get_string('attemptduration', 'assignmentques');
    }
     ...
     * @param bool $usercanseegrades whether the user is allowed to see grades for this assignmentques.
     * @param array $columns the list of columns. Added to.
     * @param array $headers the columns headings. Added to.
     * @param bool $includefeedback whether to include the feedbacktext columns
     */
    protected function add_grade_columns($assignmentques, $usercanseegrades, &$columns, &$headers, $includefeedback = true) {
        if ($usercanseegrades) {
            $columns[] = 'sumgrades';
            $headers[] = get_string('grade', 'assignmentques') . '/' .
                    assignmentques_format_grade($assignmentques, $assignmentques->grade);
        }

        if ($includefeedback && assignmentques_has_feedback($assignmentques)) {
            $columns[] = 'feedbacktext';
            $headers[] = get_string('feedback', 'assignmentques');
        }
    }
     ...
     * @param mod_assignmentques_attempts_report_options $options the display options.
    ...
    protected function set_up_table_columns($table, $columns, $headers, $reporturl,
            mod_assignmentques_attempts_report_options $options, $collapsible) {
        ...
        if (empty($currentgroup) || $this->hasgroupstudents) {
            if (optional_param('delete', 0, PARAM_BOOL) && confirm_sesskey()) {
                if ($attemptids = optional_param_array('attemptid', array(), PARAM_INT)) {
                    require_capability('mod/assignmentques:deleteattempts', $this->context);
                    $this->delete_selected_attempts($assignmentques, $cm, $attemptids, $allowedjoins);
    ...
    /**
     * Delete the assignmentques attempts
     * @param object $assignmentques the assignmentques settings. Attempts that don't belong to
     * this assignmentques are not deleted.
     ...
     */
    protected function delete_selected_attempts($assignmentques, $cm, $attemptids, \core\dml\sql_join $allowedjoins) {
        global $DB;
        ...
        foreach ($attemptids as $attemptid) {
            if (empty($allowedjoins->joins)) {
                $sql = "SELECT assignmentquesa.*
                         ...
                         WHERE assignmentquesa.id = :attemptid";
            } else {
                $sql = "SELECT assignmentquesa.*
                          FROM {assignmentques_attempts} assignmentquesa
                          JOIN {user} u ON u.id = assignmentquesa.userid
                        {$allowedjoins->joins}
                         WHERE {$allowedjoins->wheres} AND assignmentquesa.id = :attemptid";
            }
            $params = $allowedjoins->params + array('attemptid' => $attemptid);
            $attempt = $DB->get_record_sql($sql, $params, IGNORE_MULTIPLE);
            if (!$attempt || $attempt->assignmentques != $assignmentques->id || $attempt->preview != 0) {
                // Ensure the attempt exists, belongs to this assignmentques and belongs to
                // a student included in the report. If not skip.
                continue;
            }

            // Set the course module id before calling assignmentques_delete_attempt().
